...
const GoogleDriveAuthButton = ({ 
  ...
  // 🔧 FIXED: Enhanced postMessage handler với improved origin handling
  useEffect(() => {
    const handleOAuthMessage = (event) => {
      console.log('📬 Received OAuth message:', event.data);
      console.log('📍 Message origin:', event.origin);

      // 🔧 FIX: Accept messages from both localhost and 127.0.0.1 on port 8080
  ...
  // 🔧 ENHANCED: Better popup handling với comprehensive COOP workaround
  const handleOAuthPopup = async (authUrl) => {
    return new Promise((resolve, reject) => {
      ...
      // 🆕 ENHANCED: Multi-origin postMessage listener
      const handlePopupMessage = (event) => {
        // 🔧 FIX: Accept messages from OAuth callback endpoints
        const allowedOrigins = [
          'http://localhost:8080',
          'http://127.0.0.1:8080'
        ];
      ...
      // 🔧 FALLBACK: Popup close detection với better error handling
      ...
      // 🔧 TIMEOUT: Enhanced timeout với better cleanup
      setTimeout(() => {
        if (!checkCompleted) {
          checkCompleted = true;
          window.removeEventListener('message', handlePopupMessage);
  ...
  // 🔒 SECURITY: Enhanced auth result checking
  const checkAuthResult = async (maxRetries = 3) => {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.log(`🔍 Checking auth result (attempt ${attempt}/${maxRetries})...`);
        ...
        if (result.success && result.authenticated) {
          console.log('✅ Authentication confirmed:', result.user_email);
          
          // 🔒 SECURITY: Store session token
          if (result.session_token) {
            sessionStorage.setItem('session_token', result.session_token);
          }
          ...
          // 🔒 SECURITY: Pass session token instead of credentials
          if (onAuth) {
            onAuth({
              success: true,
