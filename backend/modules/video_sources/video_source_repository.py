...
class VideoSourceRepository:
    ...
    def upsert_video_source(self, source_data: Dict[str, Any]) -> Optional[int]:
        ...
        try:
            with safe_db_connection() as conn:
                ...
                self.logger.info(
                    f"🔄 Starting UPSERT operation for {source_type} source (ID: {target_source_id})"
                )
                ...
                """,
                    (
                        source_data.get(
                            "name", f"{'Local' if source_type == 'local' else 'Google'} Storage"
                        ),
    ...
    def _cleanup_source_relations(
        self, cursor, source_id: int, new_source_data: Dict[str, Any] = None
    ):
        ...
        try:
            ...
            if email_changed:
                ...
                self.logger.info(
                    f"🗑️  CLEANED download history (email changed to different account)"
                )
            else:
                # Same email → Preserve download history (avoid re-download)
            ...
            if source_type == "cloud":
                ...
                """,
                    (
                        ...
                        10,
                        "initialized",
                        "Auto-sync initialized for new source",
                    ),
                )
    ...
    def get_source_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            with safe_db_connection() as conn:
                ...
                stats["cameras"] = [
                    {"name": row[0], "folder_path": row[1], "is_selected": bool(row[2])}
    ...
    def validate_source_switch(self, new_source_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        validation_result = {"valid": True, "warnings": [], "errors": [], "impact_summary": {}}
        ...
        try:
            ...
            self.logger.info(
                f"✅ Source validation completed: {'Valid' if validation_result['valid'] else 'Invalid'}"
            )
