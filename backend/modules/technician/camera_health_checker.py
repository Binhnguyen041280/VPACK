    ...
    try:
        with safe_db_connection() as conn:
            ...
            if not isinstance(area_list, list) or len(area_list) != 4:
                logger.warning(f"[HEALTH] Invalid qr_trigger_area format for {camera_name}: {area_list}")
                return None
...
def run_health_check(
    ...
    try:
        ...
        if actual_trigger_roi:
            ...
            logger.info(f"[HEALTH] Using actual qr_trigger_area from packing_profiles for {camera_name}")
        else:
        ...
        logger.info(f"[HEALTH] Sampling from baseline window: {baseline_start:.2f}s to {baseline_start + 3.0:.2f}s")
        ...
        logger.info(f"[HEALTH] QR Success: Baseline={baseline_rate_pct:.1f}%, Current={current_rate_pct:.1f}%, Degradation={degradation_pct:.1f}%")
        ...
        logger.info(f"[HEALTH] QR Readable: {degradation_pct:.1f}% degradation → {qr_readable_status}")
        ...
        if qr_bbox_list and baseline_qr_bbox:
            ...
            logger.info(f"[HEALTH] Position: Baseline=({baseline_center[0]:.0f}, {baseline_center[1]:.0f}), Current=({current_center[0]:.0f}, {current_center[1]:.0f})")
            logger.info(f"[HEALTH] Position Offset: {offset_distance:.1f}px ({position_offset_pct:.1f}% of ROI diagonal) → {position_status}")
        ...
        logger.info(f"[HEALTH] Overall Status: QR={qr_readable_status}, Pos={position_status} → {overall_status}")
        ...
        with safe_db_connection() as conn:
            ...
            cursor.execute(
                "SELECT id FROM camera_health_check_results WHERE camera_name = ?",
                (camera_name,)
            )
            ...
            if existing:
                ...
                cursor.execute(
                    f"UPDATE camera_health_check_results SET {update_cols} WHERE id = ?",
                    values
                )
            else:
        ...
        return {
            ...
            'metrics': {
                ...
                'camera_position': {
                    ...
                    'offset_pct': position_offset_pct
                }
            }
        }
...
def check_position_offset(detected_qr_center: tuple,
                          trigger_roi: dict) -> Dict:
    """
    ...
    try:
        ...
        # ROI diagonal
        roi_diagonal = np.sqrt(
            trigger_roi['w']**2 + trigger_roi['h']**2
        )
...
def evaluate_camera_health_checklist(camera_name: str,
                                     video_path: str) -> Dict:
    """
    ...
    if not baseline:
        return {
            'success': False,
            'error': f'No baseline for {camera_name}'
        }
...
def should_run_health_check(camera_name: str) -> bool:
    ...
    try:
        with safe_db_connection() as conn:
            ...
            if should_run:
                logger.info(f"[HEALTH] {camera_name}: 2h interval passed ({hours_passed:.1f}h) - running check")
            else:
                logger.debug(f"[HEALTH] {camera_name}: Last check was {hours_passed:.1f}h ago - skipping")
