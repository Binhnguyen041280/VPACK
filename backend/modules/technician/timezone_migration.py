    ...
    try:
        with db_rwlock.gen_rlock():
            with safe_db_connection() as conn:
                ...
                cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='timezone_metadata'")
            ...
            with safe_db_connection() as conn:
                ...
                if count == 0:
                    ...
                    cursor.execute('''
                        ...
                        VALUES (?, ?, ?, ?, ?, ?)
                    ''', (user_timezone_name, 1, current_utc_timestamp, True, current_utc_timestamp, current_utc_timestamp))
                    
                    logger.info(f"✅ Created timezone_metadata table with system timezone: {user_timezone_name}")
