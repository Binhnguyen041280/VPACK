...
def select_roi(video_path: str, camera_id: str, step: str = "packing") -> Dict[str, Any]:
    ...
    try:
        logging.debug(
            f"Starting select_roi with video_path: {video_path}, camera_id: {camera_id}, step: {step}"
        )
        ...
        try:
            ...
            if step == "packing":
                original_frame_path = os.path.join(
                    CAMERA_ROI_DIR, f"camera_{camera_id}_original.jpg"
                )
                ...
                if not ret:
                    ...
                    return {
                        "success": False,
                        "error": f"Cannot save original image at {original_frame_path}",
                    }
            ...
            while True:
                ...
                roi = cv2.selectROI(
                    f"Click and drag mouse to select -{step.capitalize()} Area-",
                    current_frame,
                    showCrosshair=True,
                    fromCenter=False,
                )
                ...
                if x != x_clamped or y != y_clamped or w != w_clamped or h != h_clamped:
                    ...
                    logging.info(
                        f"  Adjusted: x={x_clamped}, y={y_clamped}, w={w_clamped}, h={h_clamped}"
                    )
                ...
                logging.debug(
                    f"Valid ROI: x={x}, y={y}, w={w}, h={h}, hand_detected: {hand_detected}"
                )
                return result
                    ...
                    if results.multi_hand_landmarks:
                        ...
                        for hand_landmarks in results.multi_hand_landmarks:
                            ...
                            mp_drawing.draw_landmarks(
                                roi_frame, hand_landmarks, mp_hands.HAND_CONNECTIONS
                            )
    ...
    try:
        ...
        try:
            ...
            for roi in rois:
                ...
                cv2.putText(
                    frame, roi_type.upper(), (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 2
                )
...
def detect_hands_at_time(
    video_path: str, time_seconds: float, roi_config: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        try:
            ...
            if not ret:
                return {"success": False, "error": f"Cannot read frame at time {time_seconds}s"}
            ...
            with mp_hands.Hands(
                ...
                logging.debug(
                    f"Hand detection complete: {hands_detected} hands detected, confidence: {overall_confidence:.2f}"
                )
...
def preprocess_video_hands(
    video_path: str, roi_config: Dict[str, Any], fps: int = 5, progress_callback=None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        try:
            ...
            logging.info(
                f"Will process {len(processing_timestamps)} frames at {fps}fps for {duration:.1f}s video"
            )
                ...
                for idx, timestamp in enumerate(processing_timestamps):
                    try:
                        ...
                        try:
                            ...
                            if cache_key in preprocessing_progress and preprocessing_progress[
                                cache_key
                            ].get("cancelled", False):
                                logging.info(
                                    f"HAND CANCELLATION DETECTED at frame {idx+1}/{len(processing_timestamps)} - FORCING SKIP TO END"
                                )

                                # ✅ FORCE SKIP TO LAST FRAME STRATEGY
                                # Process only last 3 timestamps to finish quickly
                                remaining_timestamps = processing_timestamps[
                                    max(0, len(processing_timestamps) - 3) :
                                ]
                                logging.info(
                                    f"Fast-finishing hand detection with last {len(remaining_timestamps)} frames"
                                )
                                ...
                                for final_timestamp in remaining_timestamps:
                                    try:
                                        ...
                                        if ret:
                                            ...
                                            roi_frame = frame[
                                                y_safe : y_safe + h_safe, x_safe : x_safe + w_safe
                                            ]
                                            rgb_frame = cv2.cvtColor(roi_frame, cv2.COLOR_BGR2RGB)
                                            results = hands.process(rgb_frame)
                                ...
                                logging.info(
                                    f"Fast finish completed - processed {processed_count}/{len(processing_timestamps)} frames"
                                )
                        ...
                        if results.multi_hand_landmarks and len(results.multi_hand_landmarks) > 0:
                            ...
                            for idx, hand_landmarks in enumerate(results.multi_hand_landmarks):
                                ...
                                for landmark_idx, landmark in enumerate(hand_landmarks.landmark):
                                    ...
                                    # Step 2: MediaPipe runs on ROI - transform to TC Gốc (pixel thực)
                                    x_orig = (
                                        x_safe + landmark_x_clamped * w_safe
                                    )  # Pixel thực trong video gốc
                                    y_orig = (
                                        y_safe + landmark_y_clamped * h_safe
                                    )  # Pixel thực trong video gốc
                                    ...
                                    if idx == 0 and landmark_idx == 0:
                                        logging.info(
                                            f"TC Gốc Backend Transform - Timestamp: {timestamp:.2f}s"
                                        )
                                        logging.info(
                                            f"ROI_orig: x={x_safe}, y={y_safe}, w={w_safe}, h={h_safe}"
                                        )
                                        logging.info(
                                            f"MediaPipe ROI coords: x={landmark.x:.3f}, y={landmark.y:.3f}"
                                        )
                                        if (
                                            landmark.x != landmark_x_clamped
                                            or landmark.y != landmark_y_clamped
                                        ):
                                            logging.info(
                                                f"✅ CLAMPED MediaPipe coords: x={landmark_x_clamped:.3f}, y={landmark_y_clamped:.3f}"
                                            )
                                        logging.info(
                                            f"TC Gốc pixel: x={x_orig:.1f}, y={y_orig:.1f}"
                                        )
                                        logging.info(f"Video size: {frame_width}x{frame_height}")
                                        logging.info(
                                            f"Normalized reference: x={full_x_norm:.3f}, y={full_y_norm:.3f}"
                                        )
                            ...
                            logging.debug(
                                f"Timestamp {timestamp:.2f}s: {len(landmarks_list)} hands detected"
                            )
                        ...
                        if progress_callback:
                            if new_detection:
                                ...
                                progress_callback(
                                    progress,
                                    processed_count,
                                    len(processing_timestamps),
                                    new_detection,
                                )
                            else:
                                # Update progress even if no detection found
                                progress_callback(
                                    progress, processed_count, len(processing_timestamps)
                                )
                        ...
                        if processed_count % 25 == 0:
                            ...
                            logging.info(
                                f"Processing progress: {progress:.1f}% ({processed_count}/{len(processing_timestamps)} frames), Detections: {total_detections_so_far}"
                            )
            ...
            # Final results
            logging.info(
                f"Pre-processing complete: {len(detections)} detections found in {processed_count} frames"
            )
