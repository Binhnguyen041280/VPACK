...
logging.basicConfig(
    filename=log_file_path, level=logging.DEBUG, format="%(asctime)s - %(levelname)s - %(message)s"
)
...
def select_qr_roi(video_path, camera_id, roi_frame_path, step="mvd"):
    ...
    try:
        logger.debug(
            f"[MVD] Starting select_qr_roi with video_path: {video_path}, camera_id: {camera_id}, roi_frame_path: {roi_frame_path}, step: {step}"
        )
        ...
        while table_type is None:
            ...
            window_title = (
                "**** Press 1 for standard table, 2 for non-standard table, q to exit ****"
            )
        ...
        while True:
            ...
            # Draw ROI 1 (QR code)
            window_title = (
                "**** Drag mouse to draw QR code area. Press Enter to confirm, Esc to cancel ****"
            )
            try:
                ...
                cv2.namedWindow(window_title, cv2.WINDOW_NORMAL)
                roi1 = cv2.selectROI(
                    window_title, current_frame, showCrosshair=True, fromCenter=False
                )
                cv2.destroyAllWindows()
                ...
                if w1 > 0 and h1 > 0:
                    ...
                    cv2.rectangle(
                        current_frame, (x1, y1), (x1 + w1, y1 + h1), (0, 0, 255), 2
                    )  # Màu đỏ cho MVD
                    cv2.putText(
                        current_frame,
                        "ShippingLabel",
                    ...
                    cv2.namedWindow(
                        "**** Error: Invalid ROI. Redraw QR code area ****", cv2.WINDOW_NORMAL
                    )
                ...
                if table_type == "standard":
                    ...
                    if w2 > 0 and h2 > 0:
                        ...
                        cv2.rectangle(
                            current_frame, (x2, y2), (x2 + w2, y2 + h2), (0, 0, 255), 2
                        )  # Màu đỏ
                        cv2.putText(
                            current_frame,
                            roi2_label,
                        ...
                        logger.debug(
                            f"[MVD] Selected ROI 2: x={x2}, y={y2}, w={w2}, h={h2}, type={roi_type}"
                        )
            except Exception as e:
        ...
        def process_roi(video_file, roi_index, x, y, w, h, interval=5):
            ...
            try:
                ...
                logger.error(
                    f"[MVD] OpenCV WeChatQRCode error in ROI {roi_index + 1}: {str(e)}\n{traceback.format_exc()}"
                )
                return
            ...
            try:
                ...
                logger.error(
                    f"[MVD] OpenCV VideoCapture error in ROI {roi_index + 1}: {str(e)}\n{traceback.format_exc()}"
                )
                return
            ...
            while not exit_flag.is_set():
                try:
                    ...
                    if texts:
                        ...
                        for text, box in zip(texts, points):
                            ...
                            cv2.putText(
                                roi_frame,
                                text[:20],
                                (bottom_left[0], bottom_left[1] + 30),
                                cv2.FONT_HERSHEY_SIMPLEX,
                                0.7,
                                (0, 255, 0),
                                2,
                    ...
                    cv2.putText(
                        roi_frame,
                        "Detecting QR code. Content will be displayed if found",
                    ...
                    logger.error(
                        f"[MVD] OpenCV processing error in process_roi (ROI {roi_index + 1}): {str(e)}\n{traceback.format_exc()}"
                    )
                    break
        ...
        for i, roi in enumerate(rois):
            if roi["w"] > 0 and roi["h"] > 0:
                ...
                thread = threading.Thread(
                    target=process_roi,
                    args=(video_path, i, roi["x"], roi["y"], roi["w"], roi["h"], 5),
                )
                thread.start()
        ...
        for i in range(len(frame_queues)):
            try:
                ...
                window_title = (
                    f"**** Detecting QR code. Content will be displayed if found (ROI {i + 1}) ****"
                )
        ...
        while any(thread.is_alive() for thread in threads) or any(
            not q.empty() for q in frame_queues
        ):
            for i in range(len(frame_queues)):
                try:
                    ...
                    logger.error(
                        f"[MVD] OpenCV imshow error in loop: {str(e)}\n{traceback.format_exc()}"
                    )
                    cv2.destroyAllWindows()
...
def detect_qr_at_time(
    video_path: str, time_seconds: float, roi_config: dict, cancellation_flag=None
) -> dict:
    """
    ...
    try:
        ...
        try:
            ...
            if cancellation_flag and cancellation_flag.is_set():
                logger.debug(
                    f"[QR-DETECT] Detection cancelled before frame seek at {time_seconds}s"
                )
                return {"success": False, "error": f"Detection cancelled at {time_seconds}s"}
            ...
            if cancellation_flag and cancellation_flag.is_set():
                logger.debug(
                    f"[QR-DETECT] Detection cancelled before ROI processing at {time_seconds}s"
                )
                return {"success": False, "error": f"Detection cancelled at {time_seconds}s"}
            ...
            if cancellation_flag and cancellation_flag.is_set():
                logger.debug(
                    f"[QR-DETECT] Detection cancelled before WeChat QR detection at {time_seconds}s"
                )
                return {"success": False, "error": f"Detection cancelled at {time_seconds}s"}
            ...
            if cancellation_flag and cancellation_flag.is_set():
                logger.debug(
                    f"[QR-DETECT] Detection cancelled after WeChat QR detection at {time_seconds}s"
                )
                return {"success": False, "error": f"Detection cancelled at {time_seconds}s"}
                ...
                for i, (text, box) in enumerate(zip(texts, points)):
                    if text:  # Only include non-empty decoded text
                        ...
                        if len(box) >= 4:
                            ...
                            logger.debug(
                                f"[QR-DETECT] Found QR: '{text}' at bbox({bbox_x + x}, {bbox_y + y}, {bbox_w}, {bbox_h})"
                            )
        ...
        if cancellation_flag and cancellation_flag.is_set():
            logger.debug(
                f"[QR-DETECT] Detection cancelled during processing at {time_seconds}s: {str(e)}"
            )
            return {"success": False, "error": f"Detection cancelled at {time_seconds}s"}
        else:
            logger.error(
                f"[QR-DETECT] Error in detect_qr_at_time: {str(e)}\n{traceback.format_exc()}"
            )
            return {"success": False, "error": f"Detection error: {str(e)}"}


def preprocess_video_qr(
    video_path: str, roi_config: dict, fps: int = 5, progress_callback=None
) -> dict:
    """
    ...
    try:
        ...
        try:
            ...
            logger.debug(
                f"[QR-PREPROCESS] Video info: {total_frames} frames, {video_fps} fps, {duration:.1f}s duration"
            )
        ...
        for i, timestamp in enumerate(timestamps):
            try:
                ...
                try:
                    ...
                    if cache_key in qr_preprocessing_progress and qr_preprocessing_progress[
                        cache_key
                    ].get("cancelled", False):
                        logger.info(
                            f"[QR-PREPROCESS] CANCELLATION DETECTED at frame {i+1}/{total_timestamps} - FORCING SKIP TO END"
                        )
                        ...
                        remaining_timestamps = timestamps[
                            max(0, len(timestamps) - 3) :
                        ]  # Process only last 3 frames
                        logger.info(
                            f"[QR-PREPROCESS] Fast-finishing with last {len(remaining_timestamps)} frames"
                        )
                        ...
                        logger.info(
                            f"[QR-PREPROCESS] Fast finish completed - processed {processed_count}/{total_timestamps} frames"
                        )
                ...
                if result["success"]:
                    ...
                    if result.get("qr_count", 0) > 0:
                        logger.debug(
                            f"[QR-PREPROCESS] Frame {i+1}/{total_timestamps}: {result['qr_count']} QR codes at {timestamp}s"
                        )
                else:
                    logger.warning(
                        f"[QR-PREPROCESS] Frame {i+1}/{total_timestamps}: Failed at {timestamp}s - {result.get('error')}"
                    )
                    # Still add entry with empty detections for timeline consistency
                    detections.append({"timestamp": timestamp, "qr_detections": [], "qr_count": 0})
                ...
                if progress_callback:
                    try:
                        ...
                        progress_callback(
                            ...
                            new_detections=(
                                [detection_entry] if "detection_entry" in locals() else []
                            ),
                        )
                    except Exception as callback_error:
                        if "cancelled" in str(callback_error).lower():
                            logger.info(
                                f"[QR-PREPROCESS] Processing cancelled after frame {i+1}/{total_timestamps} (timestamp {timestamp}s)"
                            )
                            return {
                                "success": False,
                                "error": f"Processing cancelled after timestamp {timestamp}s",
                            }
                        else:
                ...
                if processed_count % max(1, total_timestamps // 4) == 0:
                    progress_percent = (processed_count / total_timestamps) * 100
                    logger.info(
                        f"[QR-PREPROCESS] Progress: {processed_count}/{total_timestamps} ({progress_percent:.1f}%)"
                    )

            except Exception as e:
                if "cancelled" in str(e).lower():
                    logger.info(
                        f"[QR-PREPROCESS] Processing cancelled during timestamp {timestamp}: {str(e)}"
                    )
                    return {
                        "success": False,
                        "error": f"Processing cancelled during timestamp {timestamp}s",
                    }
                else:
                    logger.error(
                        f"[QR-PREPROCESS] Error processing timestamp {timestamp}: {str(e)}"
                    )
        ...
        logger.info(
            f"[QR-PREPROCESS] Completed: {total_qr_detections} QR detections in {processing_time:.2f}s"
        )
        ...
        if "cancelled" in str(e).lower():
            ...
            logger.error(
                f"[QR-PREPROCESS] Error in preprocess_video_qr: {str(e)}\n{traceback.format_exc()}"
            )
...
def detect_qr_from_image(image_content: str) -> dict:
    ...
    try:
        ...
        try:
            ...
            return {"success": True, "qr_detections": qr_detections, "qr_count": len(qr_detections)}
        ...
        logger.error(
            f"[QR-IMAGE] Error in detect_qr_from_image: {str(e)}\n{traceback.format_exc()}"
        )
