...
def get_csv_headers():
    ...
    if file_content:
        if not file_content.strip():
            return (
                jsonify(
                    {"error": "File CSV is empty. Please provide a valid CSV file with content."}
                ),
                400,
            )
        ...
        try:
            ...
            return (
                jsonify(
                    {
                        "error": f"Failed to read file content: {str(e)}. Ensure the content is properly formatted."
                    }
                ),
                400,
            )
        ...
        if not os.path.exists(file_path):
            return (
                jsonify(
                    {
                        "error": f"File not found at path: {file_path}. Please check the file path and try again."
                    }
                ),
                404,
            )
        ...
        try:
            ...
            return (
                jsonify(
                    {
                        "error": f"Failed to read file from path {file_path}: {str(e)}. Ensure the file is accessible and properly formatted."
                    }
                ),
                400,
            )
        ...
        return (
            jsonify(
                {
                    "error": "No file content or path provided. Please provide either file content or a valid file path."
                }
            ),
            400,
        )
    ...
    if not rows or len(rows) < 1:
        return (
            jsonify(
                {
                    "error": "CSV file has no header. Please ensure the CSV file contains at least one row with headers."
                }
            ),
            400,
        )
    ...
    if not header:
        return (
            jsonify(
                {
                    "error": "CSV file header is empty. Please ensure the first row contains valid headers."
                }
            ),
            400,
        )
...
def parse_csv():
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "error": f"Failed to parse CSV: {str(e)}. Ensure the file and column name are valid."
                }
            ),
            500,
        )
...
def query_events():
    ...
    try:
        ...
        logger.info(
            f"Validated query: {from_timestamp} to {to_timestamp}, user_tz: {tz_result['timezone']}, cameras: {len(selected_cameras)}, codes: {len(tracking_codes)}"
        )
        ...
        with db_rwlock.gen_rlock():  # Add read lock
            ...
            """
                ...
                if selected_cameras:
                    query += " AND camera_name IN ({})".format(
                        ",".join("?" * len(selected_cameras))
                    )
                ...
                for event in events:
                    try:
                        ...
                        if not tracking_codes or any(
                            code in tracking_codes_list for code in tracking_codes
                        ):
        ...
        response_data = {
            ...
            "metadata": {
                ...
                "search_criteria": {"tracking_codes": tracking_codes, "cameras": selected_cameras},
            },
        }
        ...
        return (
            jsonify(
                {
                    "error": f"Failed to query events: {str(e)}. Ensure the database is accessible and the events table exists."
                }
            ),
            500,
        )
...
def get_events():
    ...
    try:
        ...
        response_data = {
            "events": filtered_events,
            "pagination": {"limit": limit, "offset": offset, "total": len(filtered_events)},
            "metadata": {
                "time_range": {
                    "from_utc": from_timestamp,
                    "to_utc": to_timestamp,
...
def query_events_enhanced():
    ...
    try:
        ...
        response_data = {
            ...
            "pagination": {"total_count": result.total_count, "returned_count": len(result.events)},
        ...
        logger.info(
            f"Enhanced timezone query completed: {result.total_count} events in {total_time_ms:.2f}ms"
        )
...
def get_timezone_info():
    ...
    try:
        ...
        response_data = {
            ...
            "zoneinfo": {"user_timezone": current_tz_name, "using_standard_library": True},
        ...
        return jsonify({"error": f"Failed to get timezone info: {str(e)}"}), 500
    ...
    try:
        ...
        with safe_db_connection() as conn:
            ...
            if output_video_path and os.path.exists(output_video_path):
                logger.info(
                    f"✅ Event {event_id} already cut - using cached file: {os.path.basename(output_video_path)}"
                )
                return (
                    jsonify(
                        {
            ...
            def process_video():
                try:
                    ...
                    if not os.path.exists(video_file):
                        ...
                        with file_lock:
                            ...
                            if os.path.exists(video_file):
                                logger.info(
                                    f"✅ File already downloaded by another thread: {os.path.basename(video_file)}"
                                )
                            else:
                                # File still missing - proceed with download
                                logger.warning(f"⚠️ Video file missing: {video_file}")
                                logger.info(
                                    f"🔒 Acquired download lock for: {os.path.basename(video_file)}"
                                )
                                ...
                                if file_record and file_record[0]:  # Has drive_file_id
                                    ...
                                    logger.info(
                                        f"🔄 Re-downloading cloud file (force): {os.path.basename(video_file)}"
                                    )
                                    ...
                                    try:
                                        ...
                                        if not drive:
                                            raise Exception(
                                                "Failed to get Drive client - check authentication"
                                            )
                                        ...
                                        # Force download
                                        success = core.download_single_file(
                                            drive, file_info, video_file
                                        )

                                        if success and os.path.exists(video_file):
                                            logger.info(
                                                f"✅ File re-downloaded successfully: {os.path.basename(video_file)}"
                                            )
                                        else:
                                        ...
                                        raise Exception(
                                            f"Failed to re-download cloud file: {download_error}"
                                        )
                                else:
                                    raise Exception(
                                        f"Video file not found and no cloud file record exists: {video_file}"
                                    )
                    ...
                    # Parse tracking codes
                    tracking_codes = (
                        ast.literal_eval(tracking_codes_str) if tracking_codes_str else []
                    )
                    ...
                    logger.info(
                        f"Cutting event {event_id}: start={start_time:.2f}s, duration={cut_duration:.2f}s"
                    )
                    ...
                    cmd = [
                        "ffmpeg",
                        "-ss",
                        str(start_time),  # Seek BEFORE input for faster processing
                        "-i",
                        video_file,
                        "-t",
                        str(cut_duration),
                        ...
                        "-map_metadata",
                        "0",  # Preserve all metadata (creation_time, etc.)
                        "-avoid_negative_ts",
                    ...
                    for line in process.stderr:
                        ...
                        if "time=" in line:
                            ...
                            processing_tasks[task_id]["progress"] = min(
                                40 + int(time.time() % 50), 90
                            )
    ...
    try:
        ...
        with safe_db_connection() as conn:
            ...
            for row in cursor.fetchall():
                platforms.append({"name": row[0], "column": row[1]})
...
def save_platform_preference():
    ...
    try:
        ...
        logger.info(
            f"Platform preference saved: {platform_name} -> column {column_letter} ({action})"
        )
    ...
    try:
        with safe_db_connection() as conn:
            ...
            if not os.path.exists(output_dir):
                return (
                    jsonify(
                        {"success": False, "error": f"Output directory not found: {output_dir}"}
                    ),
                    404,
                )
        ...
        with db_rwlock.gen_rlock():
            with safe_db_connection() as conn:
                ...
                if not rows:
                    ...
                    return jsonify({"event_id": event_id, "qr_detections": []}), 200
...
def export_zoom_video():
    ...
    try:
        ...
        logger.info(
            f"🔍 Exporting zoom video for event {event_id}, crop={crop_params}, zoom={zoom_factor}x, qr_ts={qr_timestamp}s"
        )
        ...
        if not os.path.exists(source_video):
            return (
                jsonify(
                    {
                        "error": f"Video file not found: {source_video}. Please process the event first."
                    }
                ),
                404,
            )
        ...
        cut_video_duration = (
            float(duration_result.stdout.strip()) if duration_result.stdout.strip() else 0
        )
        ...
        cmd = [
            "ffmpeg",
            "-ss",
            str(start_time),  # Seek to start time (relative to cut video)
            "-i",
            source_video,  # Input: cut video (not original)
            "-t",
            str(cut_duration),  # Duration to cut
            "-filter:v",
            f"crop={crop_w}:{crop_h}:{crop_x}:{crop_y}",  # Crop filter
            "-c:v",
            "libx264",  # Re-encode video (required for crop)
            "-crf",
