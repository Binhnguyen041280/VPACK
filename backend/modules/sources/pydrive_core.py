...
class PyDriveCore:
        ...
        try:
            ...
            if recursive and current_depth < max_depth:
                ...
                for subfolder in subfolders:
                    ...
                    # Build relative path
                    new_relative_path = (
                        os.path.join(relative_path, subfolder_name)
                        if relative_path
                        else subfolder_name
                    )
    ...
    def check_file_exists_locally(
        self, source_id: int, drive_file_id: str, filename: str = None
    ) -> bool:
        """
    ...
    def track_downloaded_file(
        self, source_id: int, camera_name: str, file_info: Dict, local_path: str
    ):
    ...
    def sync_folder(self, drive, folder_info: Dict, base_path: str, source_id: int) -> Dict:
        ...
        try:
            ...
            folder_name = (
                folder_info.get("name", f"folder_{folder_id}")
                if isinstance(folder_info, dict)
                else f"folder_{folder_id}"
            )
            ...
            logger.info(
                f"✅ SYNC [{source_id}] Folder completed: {downloaded_count} files, {total_size/1024/1024:.1f}MB"
            )
    ...
    def sync_source(self, source_id: int) -> Dict:
        ...
        try:
            ...
            if not drive:
                return {
                    "success": False,
                    "message": "Failed to get Drive client - check authentication",
                }
