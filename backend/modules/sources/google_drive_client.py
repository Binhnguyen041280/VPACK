...
class GoogleDriveClient:
    ...
    def __init__(
        self,
        source_id=None,
        credentials_path="google_drive_credentials_web.json",
    ...
    def load_credentials_from_storage(self):
        ...
        try:
            ...
            token_filename = (
                f"google_drive_{hashlib.md5(user_email.encode()).hexdigest()[:16]}.json"
            )
    ...
    def test_connection(self):
        ...
        try:
            ...
            return {
                ...
                "pydrive2_compatible": (
                    pydrive_result.get("success", False) if pydrive_result else False
                ),
                "pydrive2_message": (
                    pydrive_result.get("message", "Not tested")
                    if pydrive_result
                    else "oauth2client credentials not available"
                ),
            }
    ...
    def upload_file(self, file_path, folder_id="root", custom_name=None):
        ...
        try:
            ...
            file = (
                self.service.files()
                .create(body=file_metadata, media_body=media, fields="id, name, size, createdTime")
                .execute()
            )
    ...
    def list_files(self, folder_id="root", limit=10):
        ...
        try:
            ...
            results = (
                self.service.files()
                .list(
                    q=query, pageSize=limit, fields="files(id, name, size, createdTime, mimeType)"
                )
                .execute()
            )
    ...
    def delete_file(self, file_id):
        ...
        try:
            ...
            return {"success": True, "message": f"File deleted successfully: {file_id}"}
