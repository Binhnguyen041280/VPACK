...
class OAuthError(VTrackError):
    ...
    def __init__(self, message: str, severity: ErrorSeverity = ErrorSeverity.HIGH, details: Dict = None):
        super().__init__(message, ErrorType.OAUTH, severity, details)

class DatabaseError(VTrackError):
    """Database operation errors"""
    def __init__(self, message: str, severity: ErrorSeverity = ErrorSeverity.MEDIUM, details: Dict = None):
        super().__init__(message, ErrorType.DATABASE, severity, details)

class FileOperationError(VTrackError):
    """File operation errors"""
    def __init__(self, message: str, severity: ErrorSeverity = ErrorSeverity.LOW, details: Dict = None):
...
class ErrorHandler:
    ...
    QUOTA_CONFIG = RetryConfig(max_retries=2, base_delay=60.0, max_delay=600.0)  # Longer delays for quota
    ...
    def classify_error(self, error: Exception) -> ErrorType:
        ...
        if any(keyword in error_msg for keyword in [
            ...
            'dns', 'socket', 'httperror', 'connectionerror'
        ]):
        ...
        # OAuth errors
            ...
            'oauth', 'token', 'credentials', 'unauthorized', 
            'invalid_grant', 'access_denied', 'authentication'
        ]):
        ...
        # Database errors
            ...
            'database is locked', 'sqlite', 'no such table', 
            'constraint', 'database', 'operational error'
        ]):
            return ErrorType.DATABASE
            
        # File operation errors
        elif any(keyword in error_msg for keyword in [
            'permission denied', 'no space left', 'file not found',
            'directory', 'disk', 'io error', 'file size mismatch'
        ]):
            return ErrorType.FILE_OPERATION
    ...
    def handle_error(self, error: Exception, context: Dict = None, source_id: int = None) -> Dict[str, Any]:
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if error_types and error_type not in error_types:
                        logger.error(f"❌ Error type {error_type.value} not in retry list, raising immediately")
                        raise
                    
                    if attempt < max_retries:
                        if handler.should_retry(error_type, attempt):
                            delay = handler.calculate_delay(error_type, attempt)
                            logger.warning(f"⏳ Retry attempt {attempt + 1}/{max_retries} in {delay:.1f}s: {str(e)}")
                            time.sleep(delay)
                        else:
                            break
                    else:
