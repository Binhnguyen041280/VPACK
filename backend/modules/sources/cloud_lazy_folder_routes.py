    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            lazy_folder_rate_limit_storage[client_ip] = [
                call_time
                for call_time in lazy_folder_rate_limit_storage[client_ip]
            ...
            if len(lazy_folder_rate_limit_storage[client_ip]) >= max_calls:
                logger.warning(
                    f"🚫 Lazy folder rate limit exceeded for {client_ip} on {endpoint_type}"
                )
                return (
                    jsonify(
                        {
                            ...
                            "retry_after": int(
                                time_window
                                - (current_time - lazy_folder_rate_limit_storage[client_ip][0])
                            ),
                        }
                    ),
                    429,
    ...
    try:
        ...
        if is_video_source_setup:
            ...
            if session_token or (auth_result and auth_result.get("credentials")):
                logger.info(
                    "🎯 Step 3 setup: Found session token/credentials - allowing folder access"
                )
...
def get_folder_depth():
    ...
    try:
        ...
        return jsonify({"success": False, "message": f"Failed to get folder depth: {str(e)}"}), 500
...
def search_folders():
    ...
    try:
        ...
        return jsonify({"success": False, "message": f"Failed to search folders: {str(e)}"}), 500
...
def clear_folder_cache():
    ...
    try:
        ...
        return (
            jsonify({"success": False, "message": f"Failed to clear folder cache: {str(e)}"}),
            500,
        )
...
def get_folder_breadcrumb():
    ...
    try:
        ...
        if not breadcrumb or breadcrumb[0]["id"] != "root":
            breadcrumb.insert(0, {"id": "root", "name": "My Drive", "depth": 0})
        ...
        return jsonify({"success": False, "message": f"Failed to get breadcrumb: {str(e)}"}), 500
...
def validate_folder_selection():
    ...
    try:
        ...
        if not folder_ids:
            return jsonify({"success": False, "message": "folder_ids array is required"}), 400
            ...
            try:
                ...
                selection_info = {
                    ...
                    "reason": (
                        "Valid camera folder"
                        if selectable
                        else f"Wrong depth (level {depth}, need level 4)"
                    ),
                }
        ...
        return (
            jsonify({"success": False, "message": f"Failed to validate selection: {str(e)}"}),
            500,
        )
