...
class PyDriveDownloader:
    ...
    def initialize_sync_status(
        self, source_id: int, sync_enabled: bool = True, interval_minutes: int = 10
    ):
    ...
    def start_auto_sync(self, source_id: int) -> bool:
        ...
        try:
            ...
            if not self.get_sync_status(source_id):
                self.initialize_sync_status(
                    source_id, sync_enabled=True, interval_minutes=DEFAULT_SYNC_INTERVAL_MINUTES
                )
        ...
        try:
            ...
            if result.get("success"):
                ...
                if any(keyword in error_msg for keyword in ["timeout", "connection", "network"]):
                    self._update_simple_status(
                        source_id, "retrying", "Đang thử lại do lỗi tạm thời"
                    )
                else:
        ...
        try:
            ...
            with safe_db_connection() as conn:
                ...
                cursor.execute(
                    ...
                    WHERE source_id = ?
                """,
                    (datetime.now().isoformat(), error_message, error_type, source_id),
                )
        except Exception as e:
        ...
        try:
            ...
            try:
                ...
                if status and status.get("sync_enabled", True):
                    ...
                    timer.start()
                    logger.debug(
                        f"⏰ Next sync scheduled for source {source_id} in {interval} minutes"
                    )
                else:
                    # Remove if disabled
