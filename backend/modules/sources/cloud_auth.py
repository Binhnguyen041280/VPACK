...
def _load_or_generate_oauth_encryption_key():
    ...
    try:
        ...
        if env_key:
            ...
            logger.warning(
                "⚠️ Using temporary encryption key - credentials won't persist across restarts"
            )
...
class CloudAuthManager:
    ...
    PROVIDER_SCOPES = {
        ...
        "gmail_only": ["openid", "email", "profile"],
    }
    ...
    def generate_session_token(
        self, user_email: str, user_info: Dict[str, Any], expires_minutes: int = 30
    ) -> Optional[str]:
        ...
        try:
            ...
            logger.info(
                f"✅ Generated session token for: {user_email} (expires in {expires_minutes}min)"
            )
            return token
        except Exception as e:
    ...
    def initiate_oauth_flow(
        self, redirect_uri: str = "http://localhost:8080/oauth/callback"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if self.provider in ["google_drive", "gmail_only"]:
                ...
                return {"success": False, "message": f"OAuth2 not implemented for {self.provider}"}
    ...
    def _initiate_google_oauth(self, redirect_uri: str) -> Dict[str, Any]:
        ...
        try:
            ...
            client_secrets_file = os.path.join(
                self.credentials_dir, self.OAUTH_ENDPOINTS[self.provider]["client_secrets_file"]
            )
    ...
    def complete_oauth_flow(
        self, session_id: str, authorization_code: str, state: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if not credentials.valid:
                return {"success": False, "message": "Invalid credentials received from OAuth flow"}
            ...
            logger.info(
                f"✅ OAuth2 flow completed securely for: {user_info.get('email', 'Unknown')}"
            )
    ...
    def _get_google_user_info(self, credentials: Credentials) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "storage_used_gb": (
                    int(quota.get("usage", 0)) / (1024**3) if quota.get("usage") else 0
                ),
                "storage_total_gb": (
                    int(quota.get("limit", 0)) / (1024**3) if quota.get("limit") else "Unlimited"
                ),
            }
    ...
    def _store_credentials_securely(
        self, credentials: Credentials, user_info: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _log_authentication_event(
        ...
        try:
            ...
            log_file = os.path.join(
                audit_log_path, f'auth_audit_{datetime.now().strftime("%Y%m")}.log'
            )
            ...
            logger.info(
                f"📝 Audit log: {event_type} for {user_email} - {'Success' if success else 'Failed'}"
            )
    ...
    def revoke_credentials(
        self, session_token: str = None, user_email: str = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if user_email:
                ...
                if os.path.exists(token_filepath):
                    ...
                    # Audit logging
                    self._log_authentication_event(
                        event_type="credentials_revoked", user_email=user_email, success=True
                    )
            else:
                ...
                for revoked_user in revoked_users:
                    self._log_authentication_event(
                        event_type="credentials_revoked_bulk", user_email=revoked_user, success=True
                    )
