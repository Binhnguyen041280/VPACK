...
class AutoSyncService:
    ...
    def start_auto_sync(self, source_config: dict) -> bool:
        ...
        if not current_status:
            pydrive_downloader.initialize_sync_status(
                source_id, sync_enabled=True, interval_minutes=10
            )
        ...
        with self.sync_locks.get(source_id, threading.Lock()):
            try:
                ...
                if not downloader:
                    return {
                        "success": False,
                        "message": f"No downloader available for source type: {source_type}",
                    }

                # Download last 24 hours
                time_range = {"from": datetime.now() - timedelta(hours=24), "to": datetime.now()}
