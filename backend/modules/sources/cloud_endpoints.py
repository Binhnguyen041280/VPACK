...
def generate_session_token(
    user_email, user_info, expires_minutes=129600
):  # 90 days = 90*24*60 = 129600 minutes
    ...
    try:
        ...
        logger.info(
            f"✅ Generated background service session token for: {user_email} (expires in {expires_minutes}min = {expires_minutes//1440} days)"
        )
        return token
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            rate_limit_storage[client_ip] = [
                call_time for call_time in rate_limit_storage[client_ip] if call_time > cutoff_time
            ]
                    ...
                    jsonify(
                        {
                            ...
                            "retry_after": int(
                                time_window - (current_time - rate_limit_storage[client_ip][0])
                            ),
                        }
                    ),
                    429,
    ...
    try:
        ...
        if action == "initiate_auth":
            CLIENT_SECRETS_FILE = os.path.join(
                os.path.dirname(__file__), "credentials/google_drive_credentials_web.json"
            )
        ...
        return jsonify({"success": False, "message": f"Cloud authentication failed: {str(e)}"}), 500
...
def gmail_authenticate():
    ...
    try:
        ...
        return jsonify({"success": False, "message": f"Gmail authentication failed: {str(e)}"}), 500
...
def gmail_oauth_callback():
    ...
    try:
        ...
        if not code or not state:
            ...
            return _create_gmail_error_page(
                "Missing OAuth parameters", "Authorization code or state missing"
            )
        ...
        if not stored_state or state != stored_state:
            ...
            return _create_gmail_error_page(
                "Security verification failed", "Please try authenticating again"
            )
        ...
        try:
            session_token = generate_session_token(
                user_info["email"], user_info, expires_minutes=129600
            )  # 90 days
...
def cloud_oauth_callback():
    ...
    try:
        ...
        flow = Flow.from_client_secrets_file(
            flow_data["client_secrets_file"], scopes=flow_data["scopes"]
        )
        flow.redirect_uri = flow_data["redirect_uri"]
        ...
        try:
            ...
            if "Scope has changed" in error_msg or "userinfo" in error_msg.lower():
                ...
                logger.info(
                    "🔄 Attempting token exchange without strict scope validation (Google auto-adds userinfo scopes)..."
                )
                ...
                try:
                    ...
                    logger.error(
                        f"❌ Google Drive token exchange failed even with scope tolerance: {retry_error}"
                    )
        ...
        session["session_token"] = session_credentials[
            "session_token"
        ]  # Store session token for backend access
...
def drive_auth_status():
    ...
    if request.method == "OPTIONS":
        ...
        response.headers.add(
            ...
            "Content-Type,Authorization,x-timezone-version,x-timezone-detection,x-client-offset",
        )
...
def initialize_folder_tree():
    ...
    try:
        ...
        if not session_token:
            return (
                jsonify(
                    {"success": False, "message": "Session token required", "requires_auth": True}
                ),
                401,
            )
...
def cloud_disconnect():
    ...
    try:
        ...
        return jsonify({"success": False, "message": f"Disconnect failed: {str(e)}"}), 500
    ...
    try:
        ...
        if credentials.access_token_expired and credentials.refresh_token:
            ...
            credential_data["expires_at"] = (
                credentials.token_expiry.isoformat() if credentials.token_expiry else None
            )
...
def set_registration_mode():
    ...
    try:
        ...
        return jsonify({"success": True, "message": "Registration mode activated"}), 200

    except Exception as e:
        logger.error(f"❌ Error setting registration mode: {e}")
        return (
            jsonify({"success": False, "message": f"Failed to set registration mode: {str(e)}"}),
            500,
        )
...
def set_setup_step():
    ...
    try:
        ...
        if step:
            ...
            return (
                jsonify({"success": True, "step": step, "message": f"Setup step set to: {step}"}),
                200,
            )
        else:
            return jsonify({"success": False, "message": "Step parameter required"}), 400

    except Exception as e:
        logger.error(f"❌ Error setting setup step: {e}")
        return jsonify({"success": False, "message": f"Failed to set setup step: {str(e)}"}), 500
...
def clear_setup_step():
    ...
    try:
        ...
        return jsonify({"success": True, "message": "Setup completed - step cleared"}), 200

    except Exception as e:
        logger.error(f"❌ Error clearing setup step: {e}")
        return jsonify({"success": False, "message": f"Failed to clear setup step: {str(e)}"}), 500
