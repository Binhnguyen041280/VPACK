...
class VTrackCloudClient:
    ...
    def __init__(self):
        ...
        # Offline fallback
        self.offline_fallback_enabled = (
            os.getenv("OFFLINE_FALLBACK_ENABLED", "true").lower() == "true"
        )
            ...
            for attempt in range(self.retry_attempts):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.info(
                            f"✅ Payment created successfully: Order {result.get('order_code')}"
                        )
                    ...
                    if attempt < self.retry_attempts - 1:
                        logger.warning(
                            f"⚠️ Request timeout, retrying... (attempt {attempt + 1}/{self.retry_attempts})"
                        )
                        continue
                    else:
                        return {
                            "success": False,
                            "error": f"Payment creation timeout after {self.retry_attempts} attempts",
                        }

                except requests.exceptions.ConnectionError as e:
                    if attempt < self.retry_attempts - 1:
                        logger.warning(
                            f"⚠️ Connection error, retrying... (attempt {attempt + 1}/{self.retry_attempts})"
                        )
                        continue
                    else:
                        return {
                            "success": False,
                            "error": f"Connection error after {self.retry_attempts} attempts: {str(e)}",
                        }

            # If we reach here, all attempts failed but no explicit return was triggered
            return {
                "success": False,
                "error": f"Payment creation failed after {self.retry_attempts} attempts",
            }
    ...
    def _check_internet_connectivity(self) -> bool:
        ...
        try:
            response = requests.get(
                "https://www.google.com/generate_204", timeout=3, allow_redirects=False
            )
    ...
    def get_packages(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if pricing_data.get("success"):
                ...
                return {
                    "success": False,
                    "error": pricing_data.get("error", "Pricing service unavailable"),
                }
    ...
    def check_trial_eligibility(self, machine_fingerprint: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for attempt in range(self.retry_attempts):
                try:
                    logger.debug(
                        f"🌐 Trial eligibility attempt {attempt + 1}/{self.retry_attempts}"
                    )
            ...
            return {
                "success": False,
                "data": {"eligible": False, "reason": "exception", "message": str(e)},
            }
        ...
        try:
            ...
            try:
                ...
                with safe_db_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(
                        "SELECT gmail_address FROM user_profiles ORDER BY last_login DESC LIMIT 1"
                    )
                    row = cursor.fetchone()
                    if row:
                        user_email = row[0]
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if result.get("success"):
                            logger.info(
                                f"✅ Trial license generated: {result.get('trial_license_key', 'unknown')[:12]}..."
                            )
                            return {
                                "success": True,
                                "data": {
                    ...
                    generation_errors.append(
                        f"Connection error (attempt {attempt + 1}): {str(e)[:50]}"
                    )
                except Exception as e:
                    generation_errors.append(
                        f"Unexpected error (attempt {attempt + 1}): {str(e)[:50]}"
                    )
            ...
            # All attempts failed
            logger.warning(
                f"⚠️ Trial license generation failed after {self.retry_attempts} attempts"
            )
            return {
                "success": False,
