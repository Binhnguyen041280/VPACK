...
def extract_package_from_license_key(license_key: str) -> dict:
    ...
    try:
        ...
        return {"product_type": "desktop", "expires_days": 365, "package_name": "Desktop Standard"}
...
def create_payment():
    ...
    try:
        ...
        try:
            ...
            if pricing_data.get("success"):
                ...
                if package_type not in available_packages:
                    logger.error(
                        f"❌ Invalid package type: {package_type}. Available: {list(available_packages.keys())}"
                    )
                    return (
                        jsonify(
                            {
                ...
                logger.warning(
                    f"⚠️ Cannot validate package {package_type} - pricing service unavailable"
                )
                # Continue without validation (fallback behavior)

        except Exception as validation_error:
            logger.warning(
                f"⚠️ Package validation failed: {str(validation_error)} - continuing anyway"
            )
            # Continue without validation (fallback behavior)
        ...
        if result.get("success"):
            ...
            return (
                jsonify(
                    {"success": False, "error": result.get("error", "Payment creation failed")}
                ),
                400,
            )
...
def get_packages():
    ...
    try:
        ...
        return jsonify({"success": False, "error": str(e), "packages": {}}), 500
    ...
    try:
        ...
        if result.get("success") and result.get("valid"):
            ...
            if validation_source == "offline":
                ...
                logger.warning(
                    f"⚠️ Offline validation used for {license_key[:12]}... - {result.get('reason')}"
                )
            else:
...
def activate_license():
    ...
    try:
        ...
        if is_obviously_invalid_license(license_key):
            return jsonify(
                {"success": False, "valid": False, "error": "Invalid license key format"}
            )
            ...
            return jsonify(
                {
                    ...
                    "valid": False,
                    "error": error_msg,
                    "activation": {"source": validation_source, "method": "license_invalid"},
                }
            )
            ...
            try:
                with safe_db_connection() as conn:
                    ...
                    if activation_count > 0:
                        logger.warning(
                            f"❌ SECURITY: License key already activated before - no re-activation allowed"
                        )
                        return jsonify(
                            {
                                "success": False,
        ...
        if not license_id:
            return (
                jsonify({"success": False, "error": "Failed to get or create license record"}),
                500,
            )
                    ...
                    if activation[0] == current_fingerprint:
                        ...
                        response_data = {
                            ...
                            "data": {
                                "license_key": license_key,
                                "customer_email": (
                                    license_record.get("customer_email", "unknown")
                                    if license_record
                                    else "unknown"
                                ),
                                "package_name": (
                                    license_record.get("product_type", "desktop")
                                    if license_record
                                    else "desktop"
                                ),
                                "expires_at": (
                                    license_record.get("expires_at") if license_record else None
                                ),
        ...
        try:
            ...
            with safe_db_connection() as conn:
                ...
                conn.commit()
                logger.info(
                    f"✅ Activation record created for license {license_id} with fingerprint {current_fingerprint[:16]}..."
                )
        ...
        response_data = {
            ...
            "data": {
                "license_key": license_key,
                "customer_email": (
                    license_record.get("customer_email", "unknown") if license_record else "unknown"
                ),
                "package_name": (
                    license_record.get("product_type", "desktop") if license_record else "desktop"
                ),
            ...
            jsonify(
                {
                    "success": False,
                    "error": str(e),
                    "activation": {"source": "error", "method": "exception"},
                }
            ),
            500,
        )
                ...
                jsonify(
                    {
                        ...
                        "activation": {"source": "database_only", "method": "not_found"},
                        "warning": {
                        ...
                        jsonify(
                            {
                                ...
                                "activation": {"source": "database_only", "method": "expired"},
                            }
                        ),
                        400,
                    )
            ...
            jsonify(
                {
                    ...
                    "activation": {"source": "database_only", "method": "failed"},
                }
            ),
            500,
        )
        ...
        with safe_db_connection() as conn:
            ...
            if not table_exists:
                ...
                return (
                    ...
                    200,
                )  # Return 200 with no license instead of 500
        ...
        with safe_db_connection() as conn:
            ...
            if license_row:
                ...
                try:
                    ...
                    if license_key:
                        ...
                        if (
                            ...
                            and validation_result.get("source") == "cloud"
                        ):
                            ...
                            logger.info(
                                f"📋 License validated via database fallback: {license_key[:12]}..."
                            )
                    else:
                ...
                try:
                    ...
                    if (
                        auto_trial_result.get("type") == "trial"
                        and auto_trial_result.get("status") == "active"
                    ):
                        # Active trial found/created
                        trial_license_data = auto_trial_result.get("license_data", {})

                        logger.info(
                            f"✅ Auto trial active: {auto_trial_result.get('days_left', 0)} days left"
                        )
                        ...
                        return jsonify(
                            {
                                ...
                                "license": {
                                    ...
                                    "customer_email": trial_license_data.get(
                                        "customer_email", "trial@local.dev"
                                    ),
                                    "package_name": trial_license_data.get(
                                        "product_type", "trial_7d"
                                    ),
                                    "package_type": trial_license_data.get(
                                        "product_type", "trial_7d"
                                    ),
                                    "expires_at": trial_license_data.get("expires_at"),
                                    "status": "active",
                                    "features": trial_license_data.get(
                                ...
                                "trial_status": {
                                    ...
                                    "machine_id": (
                                        auto_trial_result.get("machine_id", "")[:16] + "..."
                                        if auto_trial_result.get("machine_id")
                                        else ""
                                    ),
                                },
                                "system_status": {
                        ...
                        auto_trial_result.get("type") == "trial"
                        and auto_trial_result.get("status") == "expired"
                    ):
                        # Trial expired
                        ...
                        logger.warning(
                            f"⚠️ Auto trial check failed: {auto_trial_result.get('error')}"
                        )
