...
def validate_working_days(working_days: List[str]) -> Tuple[bool, str]:
    ...
    for day in working_days:
        if not isinstance(day, str) or day not in valid_days:
            return (
                False,
                f"Invalid day name: {day}. Must be one of: {', '.join(sorted(valid_days))}",
            )
        ...
        for area_type in ["packing_area", "trigger_area"]:
            if area_type in zone:
                ...
                if not isinstance(area, list) or len(area) != 4:
                    return (
                        False,
                        f"Detection zone {i+1} {area_type} must be an array of 4 numbers [x, y, w, h]",
                    )
...
def validate_timing_config(data: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    if min_time is not None and max_time is not None:
        ...
        if (max_time - min_time) < 5:
            return (
                False,
                "Maximum packing time must be at least 5 seconds greater than minimum packing time",
            )
...
def sanitize_input(
    value: Union[str, Dict, List], max_length: Optional[int] = None
) -> Union[str, Dict, List]:
    """
