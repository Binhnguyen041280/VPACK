...
class ROIVideoService:
    ...
    def stream_video_range(self, video_path: str, range_header: Optional[str] = None) -> Response:
        ...
        try:
            ...
            if not validation["valid"]:
                return Response(
                    json.dumps({"error": validation["error"], "details": validation["details"]}),
                    status=400,
            ...
            return Response(
                json.dumps(
                    {"error": f"Video streaming error: {str(e)}", "details": {"exception": str(e)}}
                ),
                status=500,
                mimetype="application/json",
            )
        ...
        try:
            ...
            if timestamp_seconds < 0 or timestamp_seconds > max_duration:
                return {
                    ...
                    "details": {"timestamp": timestamp_seconds, "max_duration": max_duration},
                }
            ...
            logger.error(
                f"Error extracting frame from {video_path} at {timestamp_seconds}s: {str(e)}"
            )
            return {
                "success": False,
                "error": f"Frame extraction error: {str(e)}",
                "details": {"exception": str(e), "timestamp": timestamp_seconds},
            }

    def extract_frame_by_number(
        self, video_path: str, frame_number: int, frame_format: str = "jpg", quality: int = 85
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if frame_number < 0 or frame_number >= max_frames:
                return {
                    ...
                    "details": {"frame_number": frame_number, "max_frames": max_frames},
                }
            ...
            return {
                "success": False,
                "error": f"Frame extraction error: {str(e)}",
                "details": {"exception": str(e), "frame_number": frame_number},
            }

    def validate_roi_coordinates(
        self, roi_data: List[Dict[str, Any]], video_width: int, video_height: int
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for i, roi in enumerate(roi_data):
                ...
                if x + w > video_width or y + h > video_height:
                    errors.append(
                        f"ROI {i}: ROI extends beyond video bounds ({x+w} > {video_width} or {y+h} > {video_height})"
                    )
                    continue
                ...
                if w < min_size or h < min_size:
                    errors.append(
                        f"ROI {i}: ROI too small (minimum {min_size}x{min_size}, got {w}x{h})"
                    )
                    continue
            ...
            return {"valid": True, "validated_rois": validated_rois, "count": len(validated_rois)}
