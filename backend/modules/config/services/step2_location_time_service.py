...
class Step2LocationTimeService:
        ...
        try:
            with safe_connection_wrapper() as conn:
                ...
                if row:
                    ...
                    try:
                        working_days = json.loads(working_days_json) if working_days_json else self.DEFAULT_WORKING_DAYS
                ...
                log_step_operation("2", "get_location_time", {"has_enhanced": has_enhanced_timezone})
                return config
    ...
    def validate_location_time_data(self, data: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        required_fields = ['country', 'timezone', 'language', 'working_days', 'from_time', 'to_time']
    ...
    def validate_timezone_enhanced(self, timezone_input: str) -> Tuple[bool, str, Optional[Dict[str, Any]]]:
        """
    ...
    def update_location_time_if_changed(self, new_data: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
        """
        ...
        try:
            ...
            timezone_valid, tz_error, timezone_data = self.validate_timezone_enhanced(sanitized_data["timezone"])
            ...
            with safe_connection_wrapper() as conn:
                ...
                """, (
                    sanitized_data["country"], sanitized_data["timezone"], sanitized_data["language"],
                    json.dumps(working_days_en), sanitized_data["from_time"], sanitized_data["to_time"],
                    timezone_data["iana_name"], timezone_data["display_name"], timezone_data["utc_offset_hours"],
                    timezone_data["format_type"], 1, timezone_data["updated_at"],
                    json.dumps(timezone_data["warnings"])
                ))
                
                conn.commit()
                
                log_step_operation("2", "update_location_time", {
                    "country": sanitized_data["country"],
                    "timezone": sanitized_data["timezone"],
