    ...
    try:
        ...
        try:
            ...
            if duration_seconds < MIN_DURATION:
                response["video_file"]["error"] = f"Video too short ({format_duration(duration_seconds)}). Minimum: 1 minute"
                response["video_file"]["valid"] = False
                response["summary"]["valid"] = False
            elif duration_seconds > MAX_DURATION:
                response["video_file"]["error"] = f"Video too long ({format_duration(duration_seconds)}). Maximum: 5 minutes"
                response["video_file"]["valid"] = False
                response["summary"]["valid"] = False
            else:
                # Video is valid
...
def validate_multiple_videos(file_paths: list, video_type: str = "traditional") -> Dict[str, Any]:
    ...
    if results["valid_files"] > 0:
        results["summary"]["average_duration_seconds"] = round(total_duration / results["valid_files"], 1)
