...
def get_step_location_time():
    ...
    try:
        ...
        if "error" in config:
            ...
            response = create_success_response(config, "Location/time configuration retrieved successfully")
        
        log_step_operation("2", "GET location-time success", {
...
def update_step_location_time():
    ...
    try:
        ...
        log_step_operation("2", "PUT location-time success", {
            "changed": result["changed"],
            "timezone": result["timezone"],
...
def validate_step_timezone():
    ...
    try:
        ...
        is_valid, validation_message, timezone_data = step2_location_time_service.validate_timezone_enhanced(timezone_input)
        ...
        log_step_operation("2", "POST timezone validate", {"valid": is_valid, "timezone": timezone_input})
...
def validate_step_working_days():
    ...
    try:
        ...
        log_step_operation("2", "POST working-days validate", {"valid": is_valid, "count": len(working_days)})
        return jsonify(response), status_code
