...
def get_step_timing():
    ...
    try:
        ...
        if "error" in config:
            ...
            response = create_success_response(config, "Timing configuration retrieved successfully")
        
        log_step_operation("5", "GET timing success", {
...
def update_step_timing():
    ...
    try:
        ...
        if not data:
            error_response = create_error_response("At least one timing field must be provided", "step5")
            return jsonify(error_response), 400
        
        # Log request details
        print(f"=== STEP 5 TIMING CONFIG UPDATE ===")
        for field in ["min_packing_time", "max_packing_time", "video_buffer", 
                     "storage_duration", "frame_rate", "frame_interval", "output_path"]:
            if field in data:
...
def validate_step_timing():
    ...
    try:
        ...
        if not data:
            error_response = create_error_response("At least one field must be provided for validation", "step5")
            return jsonify(error_response), 400
        
        # Validate via service
        is_valid, validation_message, validation_details = step5_timing_service.validate_timing_settings(data)
...
def get_step_timing_performance_estimate():
    ...
    try:
        ...
        estimates = {
            ...
            "performance_category": "high_performance" if processing_load_percentage < 25 else 
                                   "balanced" if processing_load_percentage < 50 else 
                                   "high_accuracy",
            "recommendations": []
        }
        ...
        if processing_load_percentage > 75:
            estimates["recommendations"].append("Consider increasing frame_interval to reduce processing load")
        if processing_load_percentage < 10:
            estimates["recommendations"].append("Consider decreasing frame_interval for better detection accuracy")
        if frame_interval > frame_rate:
            estimates["recommendations"].append("Frame interval should not exceed frame rate")
        
        response = create_success_response(estimates, "Performance estimates calculated successfully")
        
        log_step_operation("5", "POST timing performance-estimate success", {
        ...
        error_response, status_code = handle_general_error(e, "calculate performance estimates", "step5")
        return jsonify(error_response), status_code