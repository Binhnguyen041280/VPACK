...
def get_step_packing_area():
    ...
    try:
        ...
        if "error" in config:
            ...
            response = create_success_response(
                config, "Packing area configuration retrieved successfully"
            )

        log_step_operation(
            "4",
            "GET packing-area success",
            {
...
def get_all_cameras_roi_status():
    ...
    try:
        ...
        log_step_operation(
            "4",
            "GET all cameras ROI status success",
            {
...
def get_step_camera_packing_profile(camera_name):
    ...
    try:
        ...
        log_step_operation(
            "4",
            f"GET camera packing profile success for {camera_name}",
        ...
        error_response, status_code = handle_general_error(
            e, f"get camera profile for {camera_name}", "step4"
        )
        return jsonify(error_response), status_code
...
def step_roi_selection():
    ...
    try:
        ...
        log_step_operation(
            "4",
            "POST roi-selection",
            {"success": result.get("success", False), "camera_id": camera_id},
        )
...
def step_roi_finalization():
    ...
    try:
        ...
        result = step4_packing_area_service.call_existing_roi_finalization(
            video_path, camera_id, rois
        )
...
def get_step_packing_area_statistics():
    ...
    try:
        ...
        stats["configuration_completeness"] = (
            (stats["cameras_with_packing_area"] / max(1, stats["total_zones"])) * 100
        ...
        error_response, status_code = handle_general_error(
            e, "get packing area statistics", "step4"
        )
        return jsonify(error_response), status_code
