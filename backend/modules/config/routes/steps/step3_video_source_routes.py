...
def get_step_video_source():
    ...
    try:
        ...
        if "error" in config:
            ...
            response = create_success_response(
                config, "Video source configuration retrieved successfully"
            )

        log_step_operation(
            "3",
            "GET video-source success",
            {
        ...
        if is_valid:
            response = {
                ...
                "data": {
                    ...
                    "validationChecks": {"sourceType": "valid", "requirements": "satisfied"},
                },
            }

            # Add source-specific validation details
            if data.get("sourceType") == "local_storage":
                response["data"]["validationChecks"]["inputPath"] = (
                    "valid" if data.get("inputPath") else "missing"
                )
                response["data"]["validationChecks"]["selectedCameras"] = (
                    "valid" if data.get("selectedCameras") else "missing"
                )
            elif data.get("sourceType") == "cloud_storage":
                response["data"]["validationChecks"]["selected_tree_folders"] = (
                    "valid" if data.get("selected_tree_folders") else "missing"
                )
            ...
            response = {
                ...
                "data": {"sourceType": data.get("sourceType")},
            }
            status_code = 200  # 200 for validation endpoint, even with invalid data

        log_step_operation(
            "3",
            "POST video-source validate",
            {"valid": is_valid, "source_type": data.get("sourceType")},
        )
...
def get_step_video_source_cameras():
    ...
    try:
        ...
        camera_data = {
            ...
            "camera_count": config.get("camera_count", 0),
            "source_type": (
                config.get("current_source", {}).get("source_type")
                if config.get("current_source")
                else None
            ),
        }
        ...
        response = create_success_response(
            camera_data, "Camera configuration retrieved successfully"
        )

        log_step_operation(
            "3",
            "GET video-source cameras success",
            {
...
def get_step_video_source_sync_status():
    ...
    try:
        ...
        if config.get("current_source") and config.get("backward_compatibility"):
            ...
            pc_cameras = set(
                config.get("backward_compatibility", {}).get(
                    "processing_config_selected_cameras", []
                )
            )

            vs_path = config.get("input_path", "")
            pc_path = config.get("backward_compatibility", {}).get(
                "processing_config_input_path", ""
            )
        ...
        log_step_operation(
            "3", "GET video-source sync-status success", {"sync_status": sync_status["sync_status"]}
        )
        ...
        error_response, status_code = handle_general_error(
            e, "get video source sync status", "step3"
        )
        return jsonify(error_response), status_code
