    ...
    try:
        ...
        try:
            ...
            try:
                ...
                if active_sources:
                    ...
                    correct_working_path = get_working_path_for_source(
        ...
        try:
            with safe_db_connection() as conn:
                ...
                try:
                    cursor.execute(
                        "ALTER TABLE processing_config ADD COLUMN run_default_on_start INTEGER DEFAULT 1"
                    )
                ...
                cursor.execute(
                    "SELECT input_path, selected_cameras, frame_rate, frame_interval FROM processing_config WHERE id = 1"
                )
    ...
    if timezone_input:
        try:
            ...
            if validation_result["valid"]:
                ...
                timezone_validation_result = type(
                    "ValidationResult",
                    (),
                    {
                        "is_valid": True,
    ...
    try:
        ...
        if timezone_validation_result:
            ...
            if not timezone_validation_result.is_valid:
                response_data["timezone_validation"][
                    "error"
                ] = timezone_validation_result.error_message
        ...
        with safe_db_connection() as conn:
            ...
            if has_enhanced_columns and len(row) > 7:
                ...
                if response_data["timezone_enhanced"]["validated"]:
                    response_data["timezone_display"] = response_data["timezone_enhanced"][
                        "display_name"
                    ]
                    response_data["timezone_normalized"] = response_data["timezone_enhanced"][
                        "iana_name"
                    ]
...
def get_available_timezones():
    ...
    try:
        ...
        for tz_name in timezones[:100]:  # Limit to 100 results for performance
            tz_info = {"name": tz_name, "display_name": tz_name.replace("_", " ")}
...
def validate_timezone_endpoint():
    ...
    try:
        ...
        response_data = {
            ...
            "display_name": (
                validation_result.get("timezone", "").replace("_", " ")
                if validation_result["valid"]
                else ""
            ),
            "warnings": [],
        ...
        if current_time:
            ...
            if utc_offset_hours:
                response_data["current_offset"] = (
                    f"UTC{'+' if utc_offset_hours >= 0 else ''}{utc_offset_hours}"
                )
    ...
    try:
        ...
        if migration_result["success"]:
            response_data["message"] = (
                f"Schema migration {'simulated' if dry_run else 'completed'} successfully"
            )
    ...
    try:
        ...
        if False:  # Skip migration - already complete
            ...
            try:
                ...
                migration_result = {
                    "success": True,
                    "message": "Schema already migrated to zoneinfo",
                }
...
from .steps import step1_bp, step2_bp, step3_bp, step4_bp, step5_bp
...
def get_step_brandname():
    ...
    try:
        with safe_db_connection() as conn:
            ...
            return jsonify({"success": True, "data": {"brand_name": brand_name}}), 200
    ...
    try:
        ...
        if not re.match(r"^[a-zA-Z0-9_\-\s]+$", new_brand_name):
            return (
                jsonify(
                    {
                        "error": "Brand name can only contain letters, numbers, underscore, hyphen, and spaces"
                    }
                ),
                400,
            )
        ...
        with safe_db_connection() as conn:
            ...
            if current_brand_name == new_brand_name:
                return (
                    jsonify(
                        {
                            "success": True,
                            "data": {"brand_name": current_brand_name, "changed": False},
            ...
            return (
                jsonify(
                    {
                        "success": True,
                        "data": {"brand_name": new_brand_name, "changed": True},
...
def update_step_location_time():
    ...
    try:
        ...
        try:
            ...
            if not timezone_result["valid"]:
                return (
                    jsonify(
                        {
                            "error": f"Invalid timezone: {timezone_result.get('error', 'Unknown error')}"
                        }
                    ),
                    400,
                )
        ...
        with safe_db_connection() as conn:
                ...
                if row:
                    ...
                    try:
                        current_working_days = (
                            json.loads(current_working_days_json)
                            if current_working_days_json
                            else [
                                "Monday",
                                "Tuesday",
                                "Wednesday",
                    ...
                    default_tz = get_system_timezone_from_db()
                    current_country, current_timezone, current_language = (
                        "Vietnam",
                        default_tz,
                        "English (en-US)",
                    )
            ...
            current_working_days_json = (
                json.dumps(current_working_days)
            ...
            changes_detected = (
                ...
                or current_timezone != new_timezone
                or current_language != new_language
                or current_working_days_json != new_working_days_json
                or current_from_time != new_from_time
                or current_to_time != new_to_time
            )
...
def get_step_video_source():
    ...
    try:
        from modules.video_sources.video_source_repository import (
            get_active_source,
        ...
        if active_source:
            ...
            print(
                f"✅ Retrieved active source: {active_source['name']} (Type: {active_source['source_type']})"
            )
        ...
        try:
            with safe_db_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT input_path, selected_cameras, camera_paths FROM processing_config WHERE id = 1"
                )
                row = cursor.fetchone()

                if row:
                    proc_input_path, proc_selected_cameras_json, proc_camera_paths_json = row
                    proc_selected_cameras = (
                        json.loads(proc_selected_cameras_json) if proc_selected_cameras_json else []
                    )
                    proc_camera_paths = (
                        json.loads(proc_camera_paths_json) if proc_camera_paths_json else {}
                    )
        ...
        return jsonify({"success": True, "data": video_source_data}), 200
...
def update_step_video_source():
    ...
    try:
        ...
        db_source_type = (
            ...
            else "cloud" if source_type == "cloud_storage" else source_type
        )
        ...
        if db_source_type == "local":
            ...
            if not selected_tree_folders:
                return (
                    jsonify({"error": "At least one folder must be selected for cloud storage"}),
                    400,
                )
        ...
        processing_input_path = (
            input_path  # Default to original input_path, will be overridden for cloud
        )
        ...
        if db_source_type == "local" and detected_folders:
            ...
            try:
                with safe_db_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(
                        "SELECT gmail_address FROM user_profiles WHERE id = (SELECT MAX(id) FROM user_profiles)"
                    )
                    result = cursor.fetchone()
            ...
            if not gmail_username:
                ...
                if active_source and active_source["source_type"] == "cloud":
                    ...
                    if active_email == user_email:
                        print(
                            f"♻️  Same Gmail account: {user_email} → Reusing folder '{temp_source_name}'"
                        )
                    else:
                        print(
                            f"🔄 Different Gmail: {active_email} → {user_email} → New folder '{temp_source_name}'"
                        )
                else:
            ...
            if selected_tree_folders and len(selected_tree_folders) > 0:
                ...
                if first_camera_path and "/" in first_camera_path:
                    ...
                    if len(path_parts) > 1:
                        ...
                        print(
                            f"🎯 Extracted parent path: {temp_input_path} (from {first_camera_path})"
                        )
                    else:
            ...
            working_path = get_working_path_for_source(
                db_source_type, temp_source_name, temp_input_path
            )
            processing_input_path = (
                working_path  # FIXED: Use working_path for processing_config.input_path
            )
            ...
            for folder in selected_tree_folders:
                ...
                if folder_name:
                    ...
                    video_sources_camera_paths[folder_name] = (
                        folder_id if folder_id else folder_path
                    )
                    ...
                    print(
                        f"   - Video Sources (Google Drive): {video_sources_camera_paths[folder_name]}"
                    )
        ...
        video_sources_config_camera_paths = (
            video_sources_camera_paths
            if db_source_type == "cloud" and "video_sources_camera_paths" in locals()
            else camera_paths
        )
        ...
        if db_source_type == "cloud":
            ...
            print(
                f"✅ Video Sources camera_paths (Google Drive): {video_sources_config_camera_paths}"
            )
            ...
            with safe_db_connection() as conn:
                ...
                """,
                    (
                        ...
                        json.dumps(camera_paths),
                    ),
                )  # FIXED: Use processing_input_path (local sync path for cloud)
        ...
        if db_source_type == "cloud":
            ...
            response_data["data"][
                "originalTreeFolders"
            ] = selected_tree_folders  # Keep original data
