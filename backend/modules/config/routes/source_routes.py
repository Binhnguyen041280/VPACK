...
def test_source_connection():
    ...
    try:
        ...
        if not data:
            return (
                jsonify(
                    {"accessible": False, "message": "No data provided", "source_type": "unknown"}
                ),
                400,
            )
        ...
        if source_type == "local":
            ...
            return (
                jsonify(
                    {"accessible": is_accessible, "message": message, "source_type": source_type}
                ),
                200,
            )
        ...
        return (
            jsonify(
                {"accessible": False, "message": "Invalid JSON format", "source_type": "unknown"}
            ),
            400,
        )
...
def update_video_source(source_id):
    ...
    try:
        ...
        return (
            jsonify({"message": message, "source_id": source_id, "updated_fields": ["config"]}),
            200,
        )
...
def scan_folders():
    ...
    try:
        ...
        data = request.get_json()
        if not data:
            return jsonify({"success": False, "folders": [], "message": "No data provided"}), 400

        input_path = data.get("path")
        if not input_path:
            return (
                jsonify({"success": False, "folders": [], "message": "Path parameter is required"}),
                400,
            )
        ...
        return jsonify({"success": False, "folders": [], "message": error_message}), 500
