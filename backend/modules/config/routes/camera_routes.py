...
def debug_cameras():
    ...
    try:
        ...
        with safe_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT selected_cameras, input_path FROM processing_config WHERE id = 1"
            )
        ...
        if sources:
            ...
            if not result:
                return (
                    jsonify(
                        {
                            "error": "video_root not found in configuration. Please update via /save-config endpoint."
                        }
                    ),
                    400,
                )
...
def get_processing_cameras():
    ...
    try:
        ...
        if result and result[0]:
            selected_cameras = json.loads(result[0])
            return (
                jsonify({"selected_cameras": selected_cameras, "count": len(selected_cameras)}),
                200,
            )
        else:
            return jsonify({"selected_cameras": [], "count": 0}), 200
...
def sync_cloud_cameras():
    ...
    try:
        ...
        if all_folders:
            ...
            return (
                jsonify({"success": False, "message": "No folders found in cloud source config"}),
                400,
            )
...
def refresh_cameras():
    ...
    try:
        ...
        if not sources:
            return (
                jsonify({"success": False, "message": "No active source found", "cameras": []}),
                404,
            )
        ...
        return (
            jsonify(
                {"success": False, "error": f"Failed to refresh cameras: {str(e)}", "cameras": []}
            ),
            500,
        )
            ...
            jsonify(
                {
                    ...
                    "recommendations": {
                        "action_needed": (
                            "sync_cloud_cameras"
                            if source_info and source_info["type"] == "cloud" and not cameras_synced
                            else None
                        ),
                        "message": (
                            "Cameras not synced - use /sync-cloud-cameras endpoint"
                            if not cameras_synced
                            else "Cameras are in sync"
                        ),
                    },
