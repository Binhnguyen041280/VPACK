...
def get_file_creation_time(file_path: str, camera_name: Optional[str] = None) -> datetime:
    ...
    try:
        ...
        logger.debug(
            f"Extracted timezone-aware creation time for {file_path}: {timezone_aware_time}"
        )
...
def scan_files(
    ...
    max_ctime: Optional[datetime],
    restrict_to_current_date: bool = False,
    ...
    for root, dirs, files in os.walk(root_path):
        ...
        camera_name = (
            relative_path.split(os.sep)[0] if relative_path != "." else os.path.basename(video_root)
        )
            ...
            if file.lower().endswith(video_extensions):
                ...
                try:
                    ...
                    try:
                        ...
                        file_ctime = datetime.fromtimestamp(
                            os.path.getctime(file_path), tz=user_timezone
                        )
                    except Exception:
                        # Ultimate fallback
                        file_ctime = datetime.fromtimestamp(
                            os.path.getctime(file_path), tz=_get_system_tz()
                        )

                logger.debug(
                    f"Checking file {file_path}, ctime={file_ctime}, max_ctime={max_ctime}"
                )
                ...
                if from_time and to_time:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Failed to apply timezone-aware working hours filter for {file_path}: {e}"
                        )
                        ...
                        if not time_in_range:
                            skipped_by_ctime += 1
                            logger.debug(
                                f"Skipped file {file_path} due to time outside working hours (fallback): {file_time}"
                            )
                            continue
        ...
        if camera_ctime_map is not None:
            ...
            camera_ctime_map[camera_name] = max(
                camera_ctime_map.get(camera_name, 0), dir_ctime.timestamp()
            )
...
def save_files_to_db(
    ...
    file_ctimes: List[float],
    scan_action: str,
    days: Optional[int],
    custom_path: Optional[str],
    video_root: str,
    override_camera_name: Optional[str] = None,
) -> None:
...
def list_files(
    ...
    days,
    db_path,
    default_scan_days=None,
    camera_ctime_map=None,
    ...
    try:
            ...
            with safe_db_connection() as conn:
                ...
                if not os.path.exists(video_root):
                    try:
                        ...
                        logger.error(
                            f"Cannot create video root directory: {video_root}, error: {str(e)}"
                        )
                ...
                max_ctime = (
                    datetime.fromisoformat(last_ctime.replace("Z", "+00:00"))
                    if last_ctime
                    else datetime.now(_get_system_tz()).replace(year=1900)
                )
                ...
                cursor.execute(
                    "SELECT working_days, from_time, to_time, timezone FROM general_info WHERE id = 1"
                )
                ...
                if general_info:
                    try:
                        working_days_raw = (
                            general_info[0].encode("utf-8").decode("utf-8")
                            if general_info[0]
                            else ""
                        )
                    ...
                    from_time = (
                        datetime.strptime(general_info[1], "%H:%M").time()
                        if general_info[1]
                        else None
                    )
                    to_time = (
                        datetime.strptime(general_info[2], "%H:%M").time()
                        if general_info[2]
                        else None
                    )
                    ...
                    configured_timezone = (
                        general_info[3] if len(general_info) > 3 and general_info[3] else None
                    )
        ...
        if scan_action == "custom" and custom_path:
            ...
            if os.path.isfile(custom_path) and custom_path.lower().endswith(
            ...
            scan_days = (
                default_scan_days
                if default_scan_days is not None
                else SchedulerConfig.DEFAULT_SCAN_DAYS
            )
            time_threshold = (
                datetime.now(_get_system_tz()) - timedelta(days=scan_days)
                if is_initial_scan
                else datetime.now(_get_system_tz()) - timedelta(days=1)
            )
...
def run_file_scan(
    ...
    try:
        ...
        files, _ = list_files(
            ...
            days,
            db_path,
            camera_ctime_map=camera_ctime_map,
