...
class SchedulerConfig:
    ...
    def validate_config(cls) -> bool:
        ...
        try:
            ...
            assert (
                cls.BATCH_SIZE_MAX >= cls.BATCH_SIZE_MIN
            ), "BATCH_SIZE_MAX must be >= BATCH_SIZE_MIN"
            assert (
                cls.BATCH_SIZE_DEFAULT >= cls.BATCH_SIZE_MIN
            ), "BATCH_SIZE_DEFAULT must be >= BATCH_SIZE_MIN"
            assert (
                cls.BATCH_SIZE_DEFAULT <= cls.BATCH_SIZE_MAX
            ), "BATCH_SIZE_DEFAULT must be <= BATCH_SIZE_MAX"
            ...
            assert (
                cls.CPU_THRESHOLD_LOW < cls.CPU_THRESHOLD_HIGH
            ), "CPU_THRESHOLD_LOW must be < CPU_THRESHOLD_HIGH"
...
if not SchedulerConfig.validate_config():
    raise ValueError(
        "Invalid scheduler configuration detected. Please check the configuration values."
    )
