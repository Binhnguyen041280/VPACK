...
running_state = {"days": None, "custom_path": None, "current_running": None, "files": []}
...
def init_default_program():
    ...
    try:
        ...
        logger.info(
            f"First run completed: {first_run_completed}, Scheduler running: {scheduler.running}"
        )

        if first_run_completed and not scheduler.running:
            logger.info(
                f"Chuyển sang chế độ chạy mặc định (quét lặp) at {datetime.now(_get_log_tz()).strftime('%Y-%m-%d %H:%M:%S %Z')}"
            )
...
def program():
    ...
    """
    logger.info(
        f"POST /program called, Current state before action: scheduler_running={scheduler.running}, running_state={running_state}"
    )
    ...
    logger.info(
        f"DEBUG: Extracted - card={card}, action={action}, custom_path={custom_path}, days={days}, camera_name={camera_name}"
    )
    ...
    if action == "run":
        ...
        if card == "First Run":
            ...
            start_time = datetime.now(_get_log_tz())
            logger.info(
                f"Starting first run processing for {days} days at {start_time.strftime('%Y-%m-%d %H:%M:%S %Z')}"
            )
            ...
            try:
                ...
                logger.info(
                    f"First run scan completed at {completion_time.strftime('%Y-%m-%d %H:%M:%S %Z')}"
                )

            except Exception as e:
                error_time = datetime.now(_get_log_tz())
                logger.error(
                    f"Failed to run first scan at {error_time.strftime('%Y-%m-%d %H:%M:%S %Z')}: {str(e)}"
                )
                return jsonify({"error": f"Failed to run first scan: {str(e)}"}), 500
        # CUSTOM PROCESSING: Process specific file or directory
        elif card == "Custom":
            logger.info(
                f"DEBUG: Entering Custom processing branch with card='{card}', custom_path='{custom_path}'"
            )
            if not custom_path:
            ...
            try:
                ...
                with db_rwlock:
                    with safe_db_connection() as conn:
                        cursor = conn.cursor()
                        cursor.execute(
                            "SELECT status, is_processed FROM file_list WHERE file_path = ? AND (status = 'xong' OR is_processed = 1)",
                            (abs_path,),
                        )
                        result = cursor.fetchone()
                    if result:
            ...
            try:
                ...
                with db_rwlock:
                    with safe_db_connection() as conn:
                        cursor = conn.cursor()
                        cursor.execute(
                            "SELECT file_path FROM file_list WHERE custom_path = ? ORDER BY created_at DESC LIMIT 1",
                            (abs_path,),
                        )
                        result = cursor.fetchone()
                if result:
                    ...
                    while True:
                        with db_rwlock:
                            with safe_db_connection() as conn:
                                cursor = conn.cursor()
                                cursor.execute(
                                    "SELECT status FROM file_list WHERE file_path = ?", (result[0],)
                                )
                    ...
                    try:
                        ...
                        logger.info(
                            f"Completed Custom, transitioning to default: scheduler_running={scheduler.running}, running_state={running_state}"
                        )
                    except Exception as e:
        ...
        if card == "First Run":
            try:
                ...
                error_time = datetime.now(_get_log_tz())
                logger.error(
                    f"Error updating first_run_completed at {error_time.strftime('%Y-%m-%d %H:%M:%S %Z')}: {e}"
                )

    elif action == "stop":
        logger.info(
            f"Action stop called, current_state={running_state}, scheduler_running={scheduler.running}"
        )
        running_state["current_running"] = None
        running_state["days"] = None
        running_state["custom_path"] = None
        ...
        logger.info(
            f"State after stop: running_state={running_state}, scheduler_running={scheduler.running}"
        )

    logger.info(
        f"Program action completed: {card} {action}, final_state={running_state}, scheduler_running={scheduler.running}"
    )
    return (
        jsonify(
            {
...
def confirm_run():
    ...
    return jsonify({"program_type": scan_action}), 200
    ...
    try:
        with db_rwlock:
            with safe_db_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT file_path, status FROM file_list WHERE is_processed = 0 ORDER BY created_at DESC"
                )
    ...
    try:
        ...
        if not os.path.exists(video_root):
            ...
            return (
                jsonify(
                    {
                        "error": f"Directory {video_root} does not exist. Ensure the path is correct or create the directory."
                    }
                ),
                400,
            )
