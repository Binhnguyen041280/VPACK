...
class LicenseService:
    ...
    def _get_machine_fingerprint(self) -> str:
        ...
        try:
            ...
            try:
                ...
                try:
                    from backend.modules.license.machine_fingerprint import (
    ...
    def _check_activation_status(self, license_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            with safe_db_connection() as conn:
                ...
                other_fingerprint, other_activation_time, other_status, other_device_info = (
                    other_activation
                )
    ...
    def validate_license_comprehensive(
        self, license_key: str, strict_mode: bool = False, force_online: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if repository:
                ...
                if db_validation["valid"]:
                    ...
                    if expiry_result["valid"]:
                        ...
                        if strict_mode:
                            ...
                            validation_result["warnings"].append(
                                f"License expired: {expiry_result['message']}"
                            )
                    ...
                    if strict_mode:
                        ...
                        return validation_result
                    validation_result["warnings"].append(
                        f"Database integrity issue: {db_validation['error']}"
                    )

            # Step 5: Cloud validation (if online and requested)
            cloud_client = self._get_cloud_client()
            if cloud_client and (
                force_online or strict_mode or not validation_result.get("license_data")
            ):
                try:
                    ...
                    if cloud_result.get("success") and cloud_result.get("source") == "cloud":
                        ...
                        if strict_mode:
                            validation_result["error"] = (
                                f"Cloud validation failed: {cloud_result.get('error', 'Unknown')}"
                            )
                            return validation_result
                        validation_result["warnings"].append(
                            f"Cloud validation failed: {cloud_result.get('error', 'Unknown')}"
                    ...
                    validation_result["checks_failed"].append("cloud_validation")
                    validation_result["warnings"].append(
                        f"Cloud validation error: {str(cloud_error)}"
                    )
            ...
            critical_failures = [
                check
                for check in ["format_validation", "obvious_invalid_check"]
            ...
            if critical_failures:
                ...
                if validation_result.get("expired"):
                    ...
                    validation_result["message"] = (
                        f"License expired but data available ({validation_result['validation_source']})"
                    )
                else:
                    validation_result["status"] = LicenseStatus.VALID.value
                    validation_result["message"] = (
                        f"License valid ({validation_result['validation_source']})"
                    )
            else:
                validation_result["valid"] = False
            ...
            validation_result["validation_summary"] = {
                ...
                "validation_score": len(validation_result["checks_passed"])
                / max(1, len(validation_result["checks_performed"])),
            }
    ...
    def get_license_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": LicenseStatus.ERROR.value, "error": str(e), "license": None}
    ...
    def activate_license(self, license_key: str, force_offline: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            validation_result = self.validate_license_comprehensive(
                license_key, strict_mode=not force_offline, force_online=not force_offline
            )
            ...
            if activation_check["activation_status"] == "already_activated_this_machine":
                ...
                extra_info = {
                    ...
                    "validation_checks_passed": validation_result["validation_summary"][
                        "passed_checks"
                    ],
