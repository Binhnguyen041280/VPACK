...
class PaymentTransaction:
    ...
    @staticmethod
    def create(
        app_trans_id: str, customer_email: str, amount: int, payment_data: Optional[Dict] = None
    ) -> Optional[int]:
        try:
            with safe_db_connection() as conn:
                cursor = conn.cursor()
    ...
    def get_by_app_trans_id(app_trans_id: str) -> Optional[Dict[str, Any]]:
        try:
            with safe_db_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT * FROM payment_transactions WHERE app_trans_id = ?", (app_trans_id,)
                )
                row = cursor.fetchone()
                if not row:
                    return None
    ...
    @staticmethod
    def update_status(
        app_trans_id: str, status: str, payment_trans_id: Optional[str] = None
    ) -> bool:
        try:
            with safe_db_connection() as conn:
                cursor = conn.cursor()
                ...
                cursor.execute(
                    f"UPDATE payment_transactions SET {', '.join(update_fields)} WHERE app_trans_id = ?",
                    params,
                )
                conn.commit()
                ...
                if success:
                    logger.info(
                        f"✅ Payment transaction status updated: {app_trans_id} -> {status}"
                    )
                return success
        except Exception as e:
...
class License:
    ...
    def validate_offline(license_key: str, strict_mode: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            critical_failures = [
                check
                for check in ["format_validation", "database_integrity"]
