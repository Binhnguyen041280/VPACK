...
class LicenseRepository(BaseRepository):
    ...
    def create_license(
        ...
        try:
            ...
            expires_at = self._format_datetime(datetime.now() + timedelta(days=expires_days))
    ...
    def get_license_by_key(self, license_key: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug(
                f"✅ License found: {license_key[:12]}... for {license_data.get('customer_email')}"
            )
            return license_data
    ...
    def validate_license_integrity(self, license_key: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                row_data.get("machine_fingerprint") is not None
                or row_data.get("activation_time") is not None
            ):
            ...
            return {"valid": True, "license_data": row_data, "integrity_verified": True}
    ...
    def check_license_expiry(self, license_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"valid": False, "error": f"Expiry validation error: {str(e)}", "expired": True}
    ...
    def get_license_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            stats["total_licenses"] = (
                total_result[0]["total"] if total_result and total_result[0] else 0
            )
            ...
            active_query = """
                ...
                AND (expires_at IS NULL OR expires_at > ?)
            """
            active_result = self._execute_query_with_result(
                active_query, (current_time,), self.get_table_name()
            )
            stats["active_licenses"] = (
                active_result[0]["active"] if active_result and active_result[0] else 0
            )
            ...
            """
            expired_result = self._execute_query_with_result(
                expired_query, (current_time,), self.get_table_name()
            )
            stats["expired_licenses"] = (
                expired_result[0]["expired"] if expired_result and expired_result[0] else 0
            )
    ...
    def get_licenses_expiring_soon(self, days_threshold: int = 30) -> List[Dict[str, Any]]:
        ...
        """
        try:
            threshold_date = self._format_datetime(datetime.now() + timedelta(days=days_threshold))
            current_time = self._format_datetime(datetime.now())
            ...
            logger.info(
                f"📋 Found {len(licenses_data)} licenses expiring within {days_threshold} days"
            )
            return licenses_data
