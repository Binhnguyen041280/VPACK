...
class BaseRepository(ABC):
    ...
    def _row_to_dict(self, row: Tuple, table_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(row) != len(columns):
                logger.warning(
                    f"⚠️ Row length ({len(row)}) != columns length ({len(columns)}) for table '{table_name}'"
                )
                # Handle partial rows
    ...
    def _execute_query_with_result(
        self, query: str, params: Tuple = (), table_name: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
...
def test_repository_connection() -> bool:
    ...
    try:
        temp_repo = type(
            "TempRepo",
            (BaseRepository,),
            {"get_table_name": lambda self: "test", "get_required_fields": lambda self: []},
        )()
