        ...
        def ui_worker():
            try:
                ...
                license_key = simpledialog.askstring(
                    title="V_Track License", prompt="Enter your license key:", parent=root
                )
...
class LicenseUIManager:
    ...
    def start(self):
        ...
        if self.ui_thread is None or not self.ui_thread.is_alive():
            ...
            self.ui_thread = threading.Thread(
                target=self._ui_worker, daemon=True, name="LicenseUIManager"
            )
    ...
    def request_expiry_warning(self, days_remaining: int, callback=None):
        ...
        try:
            self.ui_queue.put(
                ("expiry_warning", {"days_remaining": days_remaining, "callback": callback}),
                timeout=1,
            )
        except queue.Full:
    ...
    def request_error_dialog(self, error_message: str, callback=None):
        ...
        try:
            self.ui_queue.put(
                ("error_dialog", {"error_message": error_message, "callback": callback}), timeout=1
            )
        except queue.Full:
