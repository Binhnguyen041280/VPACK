...
def safe_db_connection(
    timeout: int = 60, retry_attempts: int = 3, retry_delay: float = 0.5
    ...
    while attempt < retry_attempts:
        try:
            ...
            if attempt >= retry_attempts:
                ...
                raise sqlite3.Error(
                    f"Failed to connect to database after {retry_attempts} attempts: {e}"
                )
            else:
                logger.warning(
                    f"Database connection attempt {attempt} failed, retrying in {retry_delay}s: {e}"
                )
