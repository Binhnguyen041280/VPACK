...
class VideoTimezonePerformanceBenchmarks:
    ...
    def run_comprehensive_benchmarks(
        self, test_video_dir: Optional[str] = None, iterations: int = 100
    ) -> Dict[str, Any]:
        """
        ...
        """
        logger.info(
            f"🚀 Starting comprehensive timezone performance benchmarks ({iterations} iterations)"
        )
        ...
        try:
            ...
            creation_time_results = self._benchmark_creation_time_extraction(
                test_video_dir, iterations
            )
            ...
            concurrent_results = self._benchmark_concurrent_processing(
                test_video_dir, min(iterations, 20)
            )
    ...
    def _benchmark_video_scanning(self, test_dir: Optional[str], iterations: int) -> Dict[str, Any]:
        ...
        results = {"baseline_performance": {}, "enhanced_performance": {}, "comparison": {}}
        ...
        try:
            ...
            results["comparison"] = self._compare_performance_metrics(
    ...
    def _benchmark_timezone_detection(
        self, test_dir: Optional[str], iterations: int
    ) -> Dict[str, Any]:
        """Benchmark timezone detection performance."""
        if not test_dir:
            test_dir = self._create_test_video_environment()

        results = {"detection_methods": {}, "caching_impact": {}, "accuracy_metrics": {}}
    ...
    def _benchmark_creation_time_extraction(
        self, test_dir: Optional[str], iterations: int
    ) -> Dict[str, Any]:
        ...
        results = {"ffprobe_performance": {}, "enhanced_extraction": {}, "format_specific": {}}

        try:
            video_files = list(Path(test_dir).glob("**/*"))
            video_files = [f for f in video_files if f.suffix.lower() in self.test_video_formats][
                : min(20, iterations)
            ]
    ...
    def _benchmark_concurrent_processing(
        self, test_dir: Optional[str], iterations: int
    ) -> Dict[str, Any]:
        ...
        results = {"single_threaded": {}, "multi_threaded": {}, "scalability": {}}
    ...
    def _run_scanning_benchmark_baseline(
        self, test_dir: str, iterations: int
    ) -> PerformanceMetrics:
    ...
    def _run_scanning_benchmark_enhanced(
        self, test_dir: str, iterations: int
    ) -> PerformanceMetrics:
        ...
        memory_usage = end_memory - start_memory
        success_rate = (
            (files_processed) / (files_processed + errors) if (files_processed + errors) > 0 else 0
        )
        throughput = files_processed / execution_time if execution_time > 0 else 0
    ...
    def _benchmark_detection_method(
        self, video_files: List[Path], method: str, iterations: int
    ) -> PerformanceMetrics:
    ...
    def _benchmark_caching_impact(self, video_files: List[Path], iterations: int) -> Dict[str, Any]:
        ...
        for i in range(iterations):
            try:
                timezone_info = video_timezone_detector.detect_video_timezone(
                    test_file, "TestCamera"
                )
            except Exception:
                pass
    ...
    def _benchmark_detection_accuracy(self, video_files: List[Path]) -> Dict[str, Any]:
        ...
        for video_file in video_files:
            try:
                ...
                if timezone_info.detected_timezone:
                    ...
                    results["detection_sources"][source] = (
                        results["detection_sources"].get(source, 0) + 1
                    )
                else:
        ...
        results["success_rate"] = (
            results["successful_detections"] / results["total_files"]
    ...
    def _benchmark_ffprobe_extraction(
        self, video_files: List[Path], iterations: int
    ) -> PerformanceMetrics:
        ...
        for i in range(min(iterations, len(video_files))):
            try:
                ...
                cmd = [
                    ...
                    "json",
                    "-show_entries",
                    "format_tags=creation_time:format=creation_time",
                    str(video_file),
                ]
    ...
    def _benchmark_enhanced_extraction(
        self, video_files: List[Path], iterations: int
    ) -> PerformanceMetrics:
    ...
    def _benchmark_format_specific_extraction(
        self, video_files: List[Path], iterations: int
    ) -> Dict[str, Any]:
        ...
        for format_ext, files in format_groups.items():
            ...
            success_rate = (
                files_processed / (files_processed + errors)
                if (files_processed + errors) > 0
                else 0
            )
            throughput = files_processed / execution_time if execution_time > 0 else 0
    ...
    def _test_cleanup_efficiency(self, iterations: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "cleanup_efficiency": (
                memory_freed / memory_before_cleanup if memory_before_cleanup > 0 else 0
            ),
        }
    ...
    def _benchmark_scalability(self, video_files: List[Path]) -> Dict[str, Any]:
        ...
        file_counts = (
            ...
            else [1, len(video_files) // 2, len(video_files)]
        )
    ...
    def _compare_performance_metrics(
        self, baseline: PerformanceMetrics, enhanced: PerformanceMetrics
    ) -> Dict[str, Any]:
        ...
        if baseline.throughput_files_per_second > 0:
            throughput_ratio = (
                enhanced.throughput_files_per_second / baseline.throughput_files_per_second
            )
        ...
        performance_acceptable = (
            comparison.get("within_acceptable_slowdown", True)
            and comparison.get("within_acceptable_memory", True)
            and enhanced.success_rate >= baseline.success_rate
        )
        ...
        if "video_scanning" in benchmarks and "comparison" in benchmarks["video_scanning"]:
            ...
            if comparison.get("performance_acceptable", True):
                summary["performance_improvements"].append(
                    "Video scanning performance within acceptable limits"
                )
            else:
                summary["performance_issues"].append(
                    "Video scanning performance degradation detected"
                )
                summary["overall_performance"] = "needs_attention"

            summary["key_metrics"]["scanning_time_change"] = comparison.get(
                "execution_time_change", "N/A"
            )
            summary["key_metrics"]["scanning_memory_change"] = comparison.get(
                "memory_usage_change", "N/A"
            )
        ...
        if "timezone_detection" in benchmarks:
            ...
            if "caching_impact" in tz_detection:
                ...
                if cache_speedup > 2.0:
                    summary["performance_improvements"].append(
                        f"Caching provides {cache_speedup:.1f}x speedup"
                    )
    ...
    def _generate_recommendations(self, benchmarks: Dict[str, Any]) -> List[str]:
            ...
            if not comparison.get("performance_acceptable", True):
                if not comparison.get("within_acceptable_slowdown", True):
                    recommendations.append(
                        "Consider optimizing video scanning algorithm to reduce execution time"
                    )
        ...
        if "timezone_detection" in benchmarks:
            ...
            if accuracy < 0.9:
                recommendations.append(
                    "Improve timezone detection accuracy by enhancing metadata extraction"
                )

            cache_speedup = (
                benchmarks["timezone_detection"]
                .get("caching_impact", {})
                .get("cache_speedup_factor", 1.0)
            )
        ...
        if "memory_usage" in benchmarks:
            shows_leak = (
                benchmarks["memory_usage"].get("memory_growth", {}).get("shows_memory_leak", False)
            )

            if shows_leak:
                recommendations.append(
                    "Investigate and fix potential memory leaks in timezone processing"
                )

            cleanup_efficiency = (
                benchmarks["memory_usage"]
                .get("cleanup_efficiency", {})
                .get("cleanup_efficiency", 1.0)
            )
        ...
        if "concurrent_processing" in benchmarks:
            ...
            if single_threaded.get("throughput_files_per_second", 0) > multi_threaded.get(
                "throughput_files_per_second", 0
            ):
                recommendations.append(
                    "Multi-threading may not be beneficial for current workload size"
                )
...
def run_full_performance_benchmarks(
    test_dir: Optional[str] = None, iterations: int = 100
) -> Dict[str, Any]:
