...
class PathValidator:
    ...
    def check_disk_space(self, path: str, required_gb: float = 1.0) -> Dict:
        ...
        try:
            ...
            return {"sufficient": False, "available_gb": 0, "error": str(e), "path": path}
    ...
    def get_camera_paths(self, source_path: str, camera_names: List[str]) -> Dict:
        ...
        try:
            ...
            return {"success": True, "camera_paths": camera_paths, "source_path": source_path}

        except Exception as e:
            self.logger.error(f"❌ Get camera paths failed: {e}")
            return {"success": False, "error": str(e), "camera_paths": {}}
    ...
    def cleanup_unused_directories(self, source_path: str, active_cameras: List[str]) -> Dict:
        ...
        try:
            ...
            return {"success": False, "error": str(e), "removed_directories": []}
    ...
    def get_directory_health_status(self, path: str) -> Dict:
        ...
        try:
            ...
            healthy = (
                ...
                and permissions.get("writable", False)
                and disk_space.get("sufficient", False)
            )
            ...
            return {"healthy": False, "error": str(e), "path": path}
    ...
    def get_base_directories(self) -> Dict:
        ...
        for name, path in directories.items():
            try:
                ...
                status[name] = {"path": path, "exists": True, "writable": os.access(path, os.W_OK)}
            except Exception as e:
                status[name] = {"path": path, "exists": False, "error": str(e)}

        return {
            "directories": directories,
            "status": status,
            "all_healthy": all(
                s.get("exists", False) and s.get("writable", False) for s in status.values()
            ),
        }
