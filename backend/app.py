    ...
    def run(self):
        ...
        try:
            ...
            if result["action"] == "show_license_input":
                ...
                self.result_queue.put(
                    {"status": "expiring", "message": f"License expires in {days_remaining} days"}
                )
                logger.warning(f"License expires in {days_remaining} days")

            elif result["action"] == "show_error":
                self.result_queue.put(
                    {"status": "error", "message": f"License error: {result['message']}"}
                )
                logger.error(f"License error: {result['message']}")

            elif result["action"] == "continue":
                self.result_queue.put({"status": "valid", "message": "License valid"})
                logger.info("License check passed")

            else:
                self.result_queue.put(
                    {"status": "unknown", "message": f"Unknown license action: {result['action']}"}
                )

        except Exception as e:
            logger.error(f"License check failed: {e}")
            self.result_queue.put({"status": "error", "message": f"License check error: {str(e)}"})
...
def initialize_license_system():
    ...
    if not LICENSE_SYSTEM_AVAILABLE:
        ...
        license_status.update(
            {"initialized": True, "status": "disabled", "message": "License system disabled"}
        )
        return True
...
if PAYMENT_INTEGRATION_AVAILABLE:
    try:
        ...
        if connection_test.get("success"):
            logger.info(
                f"✅ Cloud connectivity verified ({connection_test.get('response_time_ms', 0)}ms)"
            )
        else:
...
def get_latest_user_wrapper():
    ...
    return cross_origin(origins=["http://localhost:3000"], supports_credentials=True)(
        get_latest_user
    )()
...
def logout_user():
    ...
    try:
        ...
        try:
            ...
            with safe_db_connection() as conn:
                ...
                if row:
                    ...
                    cursor.execute(
                        "DELETE FROM user_profiles WHERE gmail_address = ?", (user_email,)
                    )
        ...
        return cross_origin(
            origins=["http://localhost:3000", "http://localhost:3003"], supports_credentials=True
        )(lambda: {"success": True, "message": "Logged out successfully - user profile cleared"})()

    except Exception as e:
        logger.error(f"❌ Logout error: {e}")
        return cross_origin(
            origins=["http://localhost:3000", "http://localhost:3003"], supports_credentials=True
        )(lambda: {"success": False, "message": "Logout failed", "error": str(e)})()
    ...
    return jsonify(
        {
            ...
            "features": [
                ...
                (
                    "Payment Processing"
                    if PAYMENT_INTEGRATION_AVAILABLE
                    else "Payment Processing (Disabled)"
                ),
                (
                    "License Management"
                    if PAYMENT_INTEGRATION_AVAILABLE
                    else "License Management (Disabled)"
                ),
            ],
            "endpoints": {
                "health": "/health",
...
def get_license_status():
    ...
    if license_thread and license_thread.is_alive():
        ...
        if result:
            license_status.update(
                {"initialized": True, "status": result["status"], "message": result["message"]}
            )
...
def get_camera_configurations():
    ...
    try:
        ...
        return jsonify({"success": True, "cameras": cameras, "count": len(cameras)}), 200

    except Exception as e:
        logger.error(f"Error getting camera configurations: {e}")
        return jsonify({"success": False, "error": str(e), "cameras": []}), 500
...
def get_processing_status():
    ...
    try:
        ...
        return jsonify({"success": True, "processing_status": processing_status}), 200
                    ...
                    for source_id, source_name in sources:
                        try:
                            ...
                            if sync_result.get("success"):
                                ...
                                logger.warning(
                                    f"⚠️ Initial sync failed for {source_name}: {sync_result.get('message')}"
                                )
                        except Exception as e:
