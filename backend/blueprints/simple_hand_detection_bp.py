...
def process_frame():
    ...
    try:
        ...
        if time_seconds is None:
            return (
                jsonify({"success": False, "error": "Missing required parameter: time_seconds"}),
                400,
            )
        ...
        try:
            ...
            return (
                jsonify({"success": False, "error": f"Invalid time_seconds value: {str(e)}"}),
                400,
            )
        ...
        if roi_config:
            ...
            for field in required_roi_fields:
                ...
                try:
                    ...
                    return (
                        jsonify(
                            {"success": False, "error": f"Invalid ROI {field} value: {str(e)}"}
                        ),
                        400,
                    )
        ...
        result = detect_hands_at_time(
            video_path=video_path, time_seconds=time_seconds, roi_config=roi_config
        )
        ...
        if result.get("success"):
            logger.debug(
                f"Hand detection successful: {result.get('hands_detected', 0)} hands detected in {processing_time:.1f}ms"
            )
    ...
    try:
        ...
        if os.getenv("VTRACK_IN_DOCKER") == "true":
            ...
            test_video = os.path.join(
                project_root, "resources/Inputvideo/Cam1D/Cam1D_20250604_110517.mp4"
            )
        test_time = 5.0  # Test at 5 seconds

        # Test ROI (center area)
        test_roi = {"x": 200, "y": 150, "w": 400, "h": 300}

        logger.info(f"Running test detection at {test_time}s with ROI {test_roi}")

        result = detect_hands_at_time(
            video_path=test_video, time_seconds=test_time, roi_config=test_roi
        )
        ...
        logger.error(error_msg)
        return jsonify({"test_status": "failed", "error": error_msg}), 500
...
def cleanup_expired_cache():
    ...
    expired_keys = [
        key
        for key, cache_data in preprocessing_cache.items()
        ...
        def background_processing():
            try:
                ...
                def update_progress(progress, processed_count, total_frames, new_detections=None):
                    ...
                    if new_detections and len(new_detections) > 0:
                        if cache_key in preprocessing_cache:
                            ...
                            logger.debug(
                                f"Accumulated {len(new_detections)} new detections. Total: {total_detections} for {cache_key}"
                            )
                        else:
                ...
                result = preprocess_video_hands(
                    video_path, roi_config, fps, progress_callback=update_progress
                )
                ...
                if result.get("success"):
                    ...
                    logger.info(
                        f"Pre-processing completed for {cache_key}: {len(result['detections'])} detections cached"
                    )
                else:
...
def get_cached_landmarks():
    ...
    try:
        ...
        if cache_key not in preprocessing_cache:
            return jsonify({"success": False, "error": "Cache key not found or expired"}), 404
            ...
            if canvas_dims and video_dims and roi_config and closest_detection["landmarks"]:
                try:
                    ...
                    if canvas_response["success"]:
                        ...
                        logger.warning(
                            f"Canvas landmark mapping failed: {canvas_response.get('error')}"
                        )
...
def cache_info():
    ...
    try:
        ...
        total_detections = sum(
            len(cache_data.get("detections", [])) for cache_data in preprocessing_cache.values()
        )
